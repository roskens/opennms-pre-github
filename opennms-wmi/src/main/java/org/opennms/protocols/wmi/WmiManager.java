 ...
 * the resulting values against parameters. The purpose of the WMI Manager is to
 * provide poller-style functionality where low-level access to WBEM objects and
 * methods is not essential.
 * </P>
 *
...
public class WmiManager {
     ...
     *            sets the domain to connect to.
     * @param matchType
     *            the type of matching to be used for multiple results: all,
     *            none, some, one.
     */
    public WmiManager(final String host, final String user, final String pass, final String domain,
    ...
    public WmiResult performExecQuery(final WmiParams params) throws WmiException {
        ...
        } else if (params.getCompareOperation().equals("EQ") || params.getCompareOperation().equals("NEQ")
                || params.getCompareOperation().equals("GT") || params.getCompareOperation().equals("LT")) {
