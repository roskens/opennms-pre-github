...
public class WmiPlugin extends AbstractPlugin {
    ...
    public boolean isProtocolSupported(final InetAddress address, final Map<String, Object> qualifiers) {
        ...
        WmiResult result = isServer(address, agentConfig.getUsername(), agentConfig.getPassword(),
                                    agentConfig.getDomain(), matchType, agentConfig.getRetries(),
     ...
     */
    private WmiResult isServer(final InetAddress host, final String user, final String pass, final String domain,
            final String matchType, final int retries, final int timeout, final WmiParams params) {
        ...
        for (int attempts = 0; attempts <= retries && !isAServer; attempts++) {
            ...
            try {
                ...
                if (params.getWmiOperation().equals(WmiParams.WMI_OPERATION_WQL)) {
                    ...
                    LOG.debug("\\\\{}\\{} : {}", params.getWmiClass(), params.getWmiObject(),
                              WmiResult.convertStateToString(result.getResultCode()));
                }
