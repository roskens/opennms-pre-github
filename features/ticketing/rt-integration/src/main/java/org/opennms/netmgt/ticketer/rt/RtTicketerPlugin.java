...
public class RtTicketerPlugin implements Plugin {
    ...
    /**
     * {@inheritDoc} Gets ticket details from the RT trouble ticket system
     */
    @Override
    ...
    /**
     * {@inheritDoc} Creates a new ticket (if none exists) or updates an
     * existing ticket in the
     * RT trouble ticket system. Ticket updates are currently limited to
     * updating
    ...
    private void updateRtStatus(final Ticket ticket) throws PluginException {
        ...
        try {
            m_requestTracker.updateTicket(Long.valueOf(ticket.getId()), "Status: "
                    + openNMSToRTState(ticket.getState()));
        } catch (final Exception e) {
     ...
     *
     * @param state
     *            a valid <code>org.opennms.netmgt.ticketd.Ticket.State</code>.
