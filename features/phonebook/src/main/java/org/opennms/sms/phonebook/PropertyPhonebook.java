...
public class PropertyPhonebook implements Phonebook {
    ...
    /**
     * Initialize the phone book with the given property filename. The filename
     * must
     * exist either as a path (a @{link File} object) or a resource in the
     * classpath.
     *
     * @param filename
     *            the name of the phone book file
     */
    ...
    /**
     * Get the current properties map. If it exists as a file (rather than a
     * resource)
     * it will check the last modified time and only bother reading it if it's
     * changed.
     *
    ...
    private Properties getProperties() throws PhonebookException {
        ...
        try {
            ...
            if (stream == null) {
                throw new PhonebookException(String.format("Unable to find resource '%s' in the classpath.",
                                                           m_propertyFile));
            }
