...
public class DependencyLoadingRulesTest extends CorrelationRulesTestCase {
    ...
    // two kinds of needs... DependentsNeeded meaning I need to ensure the
    // things that depend on
    // component A are loaded

    // also need a DependenciesNeeded meaning I need to ensure that the things
    // that component A
    // depends on are loaded.

    // to imagine the use cases...
    // 1. component A is down so ensure that DependentsNeeded is asserted for
    // that component so
    // that all the necessary components are loaded. After this is asserted then
    // other rules
