...
public class LocationMonitorState implements Serializable, IsSerializable {
    ...
    private void handleMonitor(final GWTLocationMonitor monitor) {
        ...
        } else if (statusText.equalsIgnoreCase("REGISTERED") || statusText.equalsIgnoreCase("PAUSED")
    ...
    private StatusDetails getStatusDetailsUncached() {
        ...
        if (servicesDown.size() > 0) {
            if (servicesDown.size() == services.size()) {
                // red/down: If all started monitors report "down" for all
                // services
                return StatusDetails.down("All services are down on all started monitors.");
            } else {
                // red/down: If all started monitors report "down" for the same
                // service
                if (servicesDown.size() == 1) {
                    return StatusDetails.down(getServiceName(servicesDown.iterator().next())
                            + " has been reported down by all monitors.");
                } else {
                    return StatusDetails.down("The following services are reported down by all monitors: "
                            + getServiceNames(servicesDown) + ".");
                }
            }
        }

        // yellow/marginal: If some (but not all) started monitors report "down"
        // for the same service
        if (anyDown.size() > 0) {
            return StatusDetails.marginal("The following services are reported down by at least one monitor: "
                    + getServiceNames(anyDown) + ".");
        }
    ...
    public String toString() {
        return "LocationMonitorState[started=" + m_monitorsStarted + ",stopped=" + m_monitorsStopped + ",disconnected="
                + m_monitorsDisconnected + ",statuses=" + m_locationStatuses + ",services=" + m_serviceNames + "]";
    }
