    ...
    public void testSuccessfulLookupFailsMinimumQuality() throws Exception {
        if (shouldRun()) {
            ...
            try {
                ...
                assertEquals("response did not meet minimum quality requirement (COUNTRY is less specific than ZIP)",
                             e.getMessage());
            }
        }
    }
    ...
    public void testBadApiKey() throws Exception {
        if (shouldRun()) {
            ...
            try {
                ...
                assertEquals("an error occurred when querying MapQuest (statusCode=403, message=This is not a valid key. Please check that you have entered this correctly. If you do not have a key, you can obtain a free key by registering at http://developer.mapquest.com.)",
                             e.getMessage());
            }
        }
    }
