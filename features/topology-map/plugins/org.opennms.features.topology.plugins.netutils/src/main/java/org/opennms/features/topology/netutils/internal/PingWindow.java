...
/**
 * The PingWindow class creates a Vaadin Sub-window with a form and results
 * section
...
public class PingWindow extends Window {
    ...
    public PingWindow(final Node node, final String pingUrl) {
        ...
        requestsField.setMaxLength(4); // Max buffer of 4 to prevent buffer
                                       // overflow
        timeoutField = new TextField();
        timeoutField.setMaxLength(4); // Max buffer of 4 to prevent buffer
                                      // overflow
    ...
    protected URL buildURL() {
        ...
        if (validInput) {
            ...
            try {
                ...
                options.append("address=").append(ipDropdown.getValue()).append("&timeout=").append(timeoutField.getValue()).append("&numberOfRequests=").append(requestsField.getValue()).append("&packetSize=").append(Integer.parseInt(packetSizeDropdown.getValue().toString()) - 8);
