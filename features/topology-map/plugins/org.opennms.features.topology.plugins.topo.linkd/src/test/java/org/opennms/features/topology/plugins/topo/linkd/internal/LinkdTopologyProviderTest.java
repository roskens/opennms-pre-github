...
public class LinkdTopologyProviderTest {
    ...
    public void loadSavedGraphWithOnlyGroups() throws Exception {
        ...
        assertEquals(vert1.getParent().toString() + " ?= " + vert7, 0,
                     new RefComparator().compare(vert7, vert1.getParent()));
        assertEquals(1, m_topologyProvider.getSemanticZoomLevel(vert1));

        assertEquals("Group 32", vert2.getLabel());
        assertEquals(vert2.getParent().toString() + " ?= " + vert8, 0,
                     new RefComparator().compare(vert8, vert2.getParent()));
        assertEquals(1, m_topologyProvider.getSemanticZoomLevel(vert2));

        assertEquals("FGHKDKL", vert3.getLabel());
        assertEquals(vert3.getParent().toString() + " ?= " + vert4, 0,
                     new RefComparator().compare(vert4, vert3.getParent()));
        assertEquals(3, m_topologyProvider.getSemanticZoomLevel(vert3));

        assertEquals("Hello Again", vert4.getLabel());
        assertEquals(vert4.getParent().toString() + " ?= " + vert6, 0,
                     new RefComparator().compare(vert6, vert4.getParent()));
        assertEquals(2, m_topologyProvider.getSemanticZoomLevel(vert4));

        assertEquals("Big Group", vert5.getLabel());
        assertEquals(vert5.getParent().toString() + " ?= " + vert2, 0,
                     new RefComparator().compare(vert2, vert5.getParent()));
        assertEquals(2, m_topologyProvider.getSemanticZoomLevel(vert5));

        assertEquals("Smaller Group", vert6.getLabel());
        assertEquals(vert6.getParent().toString() + " ?= " + vert7, 0,
                     new RefComparator().compare(vert7, vert6.getParent()));
    ...
    public void testLoadSimpleGraph() throws Exception {
        ...
        for (Vertex vertex : m_topologyProvider.getVertices()) {
            ...
            // "127.0.0.1".equals(vertex.getIpAddress()) ||
            // "64.146.64.214".equals(vertex.getIpAddress()));
        }
    ...
    /**
     * Tests that the Linkdprovider does load the information stored in the xml
     * file
    ...
    public void testAssignChildrenToParentsCorrectly() throws MalformedURLException, JAXBException {
        ...
        check(topologyProvider.getVertex(namespace, "1"), m_databasePopulator.getNode1(),
              topologyProvider.getVertex(namespace, "g2"));
        check(topologyProvider.getVertex(namespace, "2"), m_databasePopulator.getNode2(), null);
        check(topologyProvider.getVertex(namespace, "3"), m_databasePopulator.getNode3(),
              topologyProvider.getVertex(namespace, "g2"));
        ...
        Assert.assertEquals(topologyProvider.getVertex(namespace, "g2").getParent(),
                            topologyProvider.getVertex(namespace, "g1"));
