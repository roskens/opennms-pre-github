...
public class KeyBufferTest {
    ...
    public void testAdd() {
        ...
        k.add(testString);
        assertEquals(1, k.size()); // Size should be 1 since we just added an
                                   // item
    }
    ...
    public void testFree() {
        ...
        assertEquals(0, k.size()); // Make sure the list is empty again after
                                   // the drain
    ...
    public void testDrain() {
        assertEquals(0, k.size()); // Make sure the list is empty before we
                                   // start
        k.add(doubleTestString); // Add the test string twice so we have some
                                 // data to drain
        assertEquals(doubleTestString, k.drain()); // drain the list into a
                                                   // string
        assertEquals(0, k.size()); // Make sure the list is empty again after
                                   // the drain
    }
