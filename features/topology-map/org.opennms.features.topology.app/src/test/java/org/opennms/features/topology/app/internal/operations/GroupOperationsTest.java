...
public class GroupOperationsTest {
    ...
    public void testABunchOfGroupOperations() {
...

        {
            ...
            // Even though we have executed the operation, it is waiting on a
            // Window
            // operation to commit the change so make sure the vertex hasn't
            // been
            // added yet.
            ...
            // Grab the window, put a value into the form field, and commit the
            // form to complete
            // the operation.
            UI window = context.getMainWindow();
            assertEquals(1, window.getWindows().size());
            ...
            for (Iterator<Component> itr = prompt.iterator(); itr.hasNext();) {
                ...
                try {
                    ...
                    // Make sure that the value was set, Vaadin will ignore the
                    // value
                    // if, for instance, the specified value is not in the
                    // Select list
        ...
        // Add a second group so that we can make sure that adding groups to
        // other groups works also
        ...
        try {
            ...
            // This should happen since a group with the same label already
            // exists
        }
        ...
        // Now move it back to szl 1 by removing it from its parent group
        {
            ...
            // Make sure that the vertex is back at the top level of the
            // hierarchy
    ...
    private static void renameGroup(GraphContainer graphContainer, Vertex group, String newLabel) {
        ...
        // Grab the window, put a value into the form field, and commit the form
        // to complete
        // the operation.
        UI window = context.getMainWindow();
        assertEquals(1, window.getWindows().size());
        ...
        for (Iterator<Component> itr = prompt.iterator(); itr.hasNext();) {
            ...
            try {
                ...
                // Make sure that the value was set, Vaadin will ignore the
                // value
                // if, for instance, the specified value is not in the Select
                // list
    ...
    public void testAddGroupToItself() {
        ...
        // we try to add the group to itself. There is no selection
        {
            ...
            Field field = form.getField("Group");
            field.setValue(group1.getId());
            Assert.assertEquals(group1.getId(), field.getValue()); // Make sure
                                                                   // that the
                                                                   // value was
                                                                   // set
            try {
        ...
        // we try to add the group to itself. There are multiple selections
        {
            ...
            EasyMock.expect(selectionManager.getSelectedVertexRefs()).andReturn(Arrays.asList((VertexRef) vertex1,
                                                                                              vertex2, group1)).anyTimes();
            EasyMock.expect(graphContainer.getSelectionManager()).andReturn(selectionManager).anyTimes();
            graphContainer.redoLayout();
            EasyMock.expectLastCall().anyTimes();
            EasyMock.replay(graphContainer);
    ...
    public void testCreateGroupOperation() {
...

        {
            ...
            assertNull("Value should be null: " + m_topologyProvider.getParent(vertex1),
                       m_topologyProvider.getParent(vertex1));
            assertNull("Value should be null: " + vertex2.getParent(), vertex2.getParent());
            assertNull("Value should be null: " + m_topologyProvider.getParent(vertex2),
                       m_topologyProvider.getParent(vertex2));
...

        {
            ...
            for (Iterator<Component> itr = prompt.iterator(); itr.hasNext();) {
                ...
                try {
                    ...
                    try {
                        ...
                        // This is expected since the new group's label collides
            ...
            // Make sure that the parent hasn't changed after the failed create
            // operation
     ...
     * </ol>
     * We add group2 to group1 and the nodes of group2 should not be added to
     * group1.
    ...
    public void testAddGroupWithNodesToAnotherGroup() {
        ...
        // we try to add group2 to group1
        {
            ...
            SelectionManager selectionManager = EasyMock.createNiceMock(SelectionManager.class);
            EasyMock.expect(selectionManager.isVertexRefSelected(EasyMock.anyObject(VertexRef.class))).andReturn(true).anyTimes();
            EasyMock.expect(selectionManager.getSelectedVertexRefs()).andReturn(Arrays.asList((VertexRef) node1, node2,
                                                                                              group2)).anyTimes();
            EasyMock.expect(graphContainer.getSelectionManager()).andReturn(selectionManager).anyTimes();
            graphContainer.redoLayout();
            EasyMock.expectLastCall().anyTimes();
            ...
            // verify
            Assert.assertEquals(group1, group2.getParent()); // group2 should be
                                                             // child of group1
            Assert.assertEquals(group2, node1.getParent()); // node1 is still
                                                            // child of group2
            Assert.assertEquals(group2, node2.getParent()); // node 2 is still
                                                            // child of group2
