...
public class TcaCollectionSet implements CollectionSet {
    ...
    protected void collect() throws CollectionException {
        try {
            ...
            SnmpWalker walker = SnmpUtils.createWalker(m_agent.getAgentConfig(),
                                                       "TcaCollector for " + m_agent.getHostAddress(), tracker);
            walker.start();
            ...
            throw new CollectionWarning("Collection of node TCA data for interface " + m_agent.getHostAddress()
                    + " interrupted: " + e, e);
        } catch (Exception e) {
     ...
     * Process.
     * <p>
     * A sample TCA Data looks like the following:
     * </p>
     * <ul>
     * <li>OID=.1.3.6.1.4.1.27091.3.1.6.1.1.172.19.37.60.1, Type=OctetString,
     * Value=172.19.37.60</li>
     * <li>OID=.1.3.6.1.4.1.27091.3.1.6.1.2.172.19.37.60.1, Type=OctetString,
     * Value=
     * |25|1327451762,11,0,11,0,1|1327451763,11,0,11,0,1|1327451764,11,0,11
     * ,0,1|1327451765,11,0,11,0,1|1327451766,11,0,11,0,1|
     * 1327451767,11,0,11,0,1
     * |1327451768,11,0,11,0,1|1327451769,11,0,11,0,1|1327451770
     * ,11,0,11,0,1|1327451771,11,0,11,0,1|
     * 1327451772,11,0,11,0,1|1327451773,11,
     * 0,11,0,1|1327451774,11,0,11,0,1|1327451775
     * ,11,0,11,0,1|1327451776,11,0,11,0,1|
     * 1327451777,11,0,11,0,1|1327451778,11,
     * 0,11,0,1|1327451779,11,0,11,0,1|1327451780
     * ,11,0,11,0,1|1327451781,11,0,11,0,1|
     * 1327451782,11,0,11,0,1|1327451783,11,
     * 0,11,0,1|1327451784,11,0,11,0,1|1327451785
     * ,11,0,11,0,1|1327451786,11,0,11,0,1|</li>
     * </ul>
     * <ul>
    ...
    private void process(TcaData tracker) throws Exception {
        ...
        AttributeGroupType attribGroupType = new AttributeGroupType(TcaCollectionResource.RESOURCE_TYPE_NAME, "all"); // It
                                                                                                                      // will
                                                                                                                      // be
                                                                                                                      // treated
                                                                                                                      // like
        ...
        for (TcaDataEntry entry : tracker.getEntries()) {
            ...
            for (int i = 0; i < samples; i++) {
                ...
                if (timestamp > lastTimestamp) {
                    ...
                    resource.setAttributeValue(new TcaCollectionAttributeType(attribGroupType, entryObjId,
                                                                              INBOUND_DELAY), rawEntry[1]);
                    resource.setAttributeValue(new TcaCollectionAttributeType(attribGroupType, entryObjId,
                                                                              INBOUND_JITTER), rawEntry[2]);
                    resource.setAttributeValue(new TcaCollectionAttributeType(attribGroupType, entryObjId,
                                                                              OUTBOUND_DELAY), rawEntry[3]);
                    resource.setAttributeValue(new TcaCollectionAttributeType(attribGroupType, entryObjId,
                                                                              OUTBOUND_JITTER), rawEntry[4]);
                    resource.setAttributeValue(new TcaCollectionAttributeType(attribGroupType, entryObjId,
                                                                              TIMESYNC_STATUS), rawEntry[5]);
    ...
    private void setLastTimestamp(TcaCollectionResource resource, long timestamp) throws Exception {
        ResourceTypeUtils.updateStringProperty(resource.getResourceDir(m_rrdRepository), Long.toString(timestamp),
                                               LAST_TIMESTAMP);
    }
}
