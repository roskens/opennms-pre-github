...
public class TcaDataCollectionConfig implements Serializable, Comparable<TcaDataCollectionConfig> {
    ...
    public RrdRepository buildRrdRepository(String collectionName) {
        ...
        repo.setHeartBeat(rrd.getStep()); // Heartbeat should be equal to the
                                          // step.
        return repo;
    }
    ...
    public int compareTo(TcaDataCollectionConfig obj) {
        return new CompareToBuilder().append(getRrdRepository(), obj.getRrdRepository()).append(getTcaDataCollections().toArray(OF_DATA_COLLECTIONS),
                                                                                                obj.getTcaDataCollections().toArray(OF_DATA_COLLECTIONS)).toComparison();
    }
    ...
    public boolean equals(Object obj) {
        if (obj instanceof TcaDataCollectionConfig) {
            ...
            return new EqualsBuilder().append(getRrdRepository(), other.getRrdRepository()).append(getTcaDataCollections().toArray(OF_DATA_COLLECTIONS),
                                                                                                   other.getTcaDataCollections().toArray(OF_DATA_COLLECTIONS)).isEquals();
        }
        return false;
    }
}
