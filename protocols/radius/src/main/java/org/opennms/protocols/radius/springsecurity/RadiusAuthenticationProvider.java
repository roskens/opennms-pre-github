...
/**
 * An org.springframework.security.providers.AuthenticationProvider
 * implementation that provides integration with a Radius server.
 *
 * @author Paul Donohue
 */
public class RadiusAuthenticationProvider extends AbstractUserDetailsAuthenticationProvider {
    ...
    protected void doAfterPropertiesSet() throws Exception {
        ...
        // Assert.notNull(this.authTypeClass,
        // "A RadiusAuthenticator object must be supplied in authTypeClass");
     ...
     *            (defaults to PAPAuthenticator)
     */
    public void setAuthTypeClass(RadiusAuthenticator authTypeClass) {
        if (authTypeClass instanceof PAPAuthenticator) {
            // There is a bug in PAPAuthenticator() that prevents instances of
            // the class
            // from being reused. Set the authenticator class to null to work
            // around this
            // problem.
     ...
     * with a comma-separated list of authorities (roles) to be assigned to the
     * user
     * If this is not set, or if the specified attribute is not found in the
     * reply
    ...
    /*
     * (non-Javadoc)
     * @see org.springframework.security.providers.dao.
    ...
    protected UserDetails retrieveUser(String username, UsernamePasswordAuthenticationToken token)
        ...
        try {
            ...
            LOG.debug("Sending AccessRequest message to {}:{} using {} protocol with timeout = {}, retries = {}, attributes:\n{}",
                      InetAddressUtils.str(serverIP), port,
                      (authTypeClass == null ? "PAP" : authTypeClass.getAuthName()), timeout, retries,
        ...
        if (!StringUtils.hasLength(rolesAttribute)) {
            ...
            if (roles == null) {
                LOG.info("Radius attribute {} not found, using default roles ({}) for user {}", rolesAttribute,
