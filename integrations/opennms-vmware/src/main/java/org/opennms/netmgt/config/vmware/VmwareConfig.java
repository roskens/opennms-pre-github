...
public class VmwareConfig implements java.io.Serializable {
    ...
    public boolean equals(final java.lang.Object obj) {
        if (obj instanceof VmwareConfig) {
            ...
            return new EqualsBuilder().append(getVmwareServer(), other.getVmwareServer()).isEquals();
        }
        return false;
    }
    ...
    public org.opennms.netmgt.config.vmware.VmwareServer getVmwareServer(final int index)
        ...
        if (index < 0 || index >= this._vmwareServerList.size()) {
            throw new IndexOutOfBoundsException("getVmwareServer: Index value '" + index + "' not in range [0.."
                    + (this._vmwareServerList.size() - 1) + "]");
        }
    ...
    public void setVmwareServer(final int index, final org.opennms.netmgt.config.vmware.VmwareServer vVmwareServer)
        ...
        // check bounds for index
        if (index < 0 || index >= this._vmwareServerList.size()) {
            throw new IndexOutOfBoundsException("setVmwareServer: Index value '" + index + "' not in range [0.."
                    + (this._vmwareServerList.size() - 1) + "]");
        }
     ...
     *
     * @param vVmwareServerList
     *            the Vector to copy.
     */
    public void setVmwareServer(final java.util.List<org.opennms.netmgt.config.vmware.VmwareServer> vVmwareServerList) {
        // copy vector
        this._vmwareServerList.clear();
