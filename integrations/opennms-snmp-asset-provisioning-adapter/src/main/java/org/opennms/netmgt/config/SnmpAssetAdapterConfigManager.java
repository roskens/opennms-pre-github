...
public class SnmpAssetAdapterConfigManager implements SnmpAssetAdapterConfig {
     ...
     *
     * @return a
     *         {@link org.opennms.netmgt.config.snmpAsset.adapter.SnmpAssetAdapterConfiguration}
     *         object.
     */
    ...
    /**
     * Returns all {@link AssetField} objects that are in packages that match
     * the specified
     * sysoid precisely based on the <sysoid> tag or by starting with the
     * content of the
     * <sysoidMask> tag.
    ...
    public AssetField[] getAssetFieldsForAddress(final InetAddress address, final String sysoid) {
        ...
        try {
            if (sysoid == null) {
                // If the sysoid is null, we won't be able to fetch any SNMP
                // attributes;
                // return an empty list.
                LOG.debug("getAssetFieldsForAddress: SNMP sysoid was null for address {}, returning empty list",
            ...
            for (final org.opennms.netmgt.config.snmpAsset.adapter.Package pkg : m_config.getPackageCollection()) {
                ...
                if (pkgSysoid != null) {
                    ...
                    LOG.warn("getAssetFieldsForAddress: Unexpected condition: both sysoid and sysoidMask are null on package {}",
                             pkg.getName());
                }
            }
            if (retval.size() == 0) {
                LOG.debug("getAssetFieldsForAddress: Zero AssetField matches returned for {} with sysoid: {}",
                          InetAddressUtils.str(address), sysoid);
            }
