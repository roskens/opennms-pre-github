...
public class SnmpAssetProvisioningAdapter extends SimplerQueuedProvisioningAdapter implements InitializingBean {
    ...
    /**
     * Creating a custom schedule for this adapter. We need to make sure that
     * the node has a system object ID set
    ...
    AdapterOperationSchedule createScheduleForNode(int nodeId, AdapterOperationType adapterOperationType) {
        ...
        LOG.info("createScheduleForNode: Scheduling {} for nodeid {} with schedule: {}", aos, adapterOperationType,
                 nodeId);
        return aos;
    }
        ...
        try {
            for (final AssetField field : m_config.getAssetFieldsForAddress(ipaddress, node.getSysObjectId())) {
                try {
                    ...
                    // This exception is thrown if the SNMP operation fails or
                    // an incorrect number of
                    // parameters is returned by the agent or because of a
                    // misconfiguration.
                    LOG.warn("doAdd: Could not set value for asset field \" {} \": {}", field.getName(),
                             e.getMessage(), e);
                }
            }
        } finally {
    ...
    private static String fetchSnmpAssetString(final SnmpAgentConfig agentConfig, final MibObjs mibObjs,
        ...
        if (values.length == aliases.size()) {
            ...
            for (int i = 0; i < values.length; i++) {
                // If the value is a NO_SUCH_OBJECT or NO_SUCH_INSTANCE error,
                // then skip it
                if (values[i].isError()) {
            ...
            LOG.warn("fetchSnmpAssetString: Invalid number of SNMP parameters returned: {} != {}", aliases.size(),
                     values.length);
            throw new MissingFormatArgumentException(
                                                     "fetchSnmpAssetString: Invalid number of SNMP parameters returned: "
                                                             + values.length + " != " + aliases.size());
        }
    }
            ...
            for (AssetField field : m_config.getAssetFieldsForAddress(ipaddress, node.getSysObjectId())) {
                try {
                    ...
                    try {
                        ...
                        LOG.warn("doUpdate: Could not set property \" {} \" on asset object: {}", field.getName(),
                                 e.getMessage(), e);
                    }
                    ...
                    // parameters is returned by the agent or because of a
                    // misconfiguration.
                    LOG.warn("doUpdate: Could not set value for asset field \" {} \": {}", field.getName(),
                             e.getMessage(), e);
                }
            }
        } finally {
