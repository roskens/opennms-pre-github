    ...
    public void handleInterfaceDown(Event e) {
        try {
            ...
            if (nodeHasEndPointService(nodeId)) {
                ...
                LOG.debug("Discarding Event {} since ip {} is node the primary interface of node {}", e.getUei(),
                          e.getInterface(), e.getNodeid());
            }
        } catch (Throwable t) {
    ...
    public void handleInterfaceUp(Event e) {
        try {
            ...
            if (nodeHasEndPointService(nodeId)) {
                ...
                LOG.debug("Discarding Event {} since ip {} is not the primary interface of node {}", e.getUei(),
                          e.getInterface(), e.getNodeid());
            }
        } catch (Throwable t) {
    ...
    public void handleServiceUnresponsive(Event e) {
        try {
            ...
            if (e.getService() != null && !e.getService().equals(getEndPointTypeValidator().getServiceName())) {
                LOG.debug("Discarding Event {} since service {} does not match EndPoint service {}", e.getUei(),
                          e.getService(), getEndPointTypeValidator().getServiceName());
                return;
            }
            int nodeId = Long.valueOf(e.getNodeid()).intValue();
