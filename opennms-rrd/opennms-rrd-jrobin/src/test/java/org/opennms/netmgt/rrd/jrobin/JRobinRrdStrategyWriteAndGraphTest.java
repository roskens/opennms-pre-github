 ...
 */
public class JRobinRrdStrategyWriteAndGraphTest {
	...
	RrdDef def = m_strategy.createDefinition("me", "/tmp", "icmp", 300, Collections.singletonList(ds), Arrays.asList(rraList));
	if (def != null) {
	...
	final int daysToGraph = 3;
        ...
        Metric metric = bigSineGauge("/tmp/icmp" , "icmp", start-step, step);

        Timer updateTime = new Timer();

        updateTime.start();
	RrdDb rrd = m_strategy.openFile("/tmp/icmp" + m_strategy.getDefaultFileExtension());
	...
	new File("/tmp/icmp.jrb").deleteOnExit();
	...
	graphWrite.start();
	FileOutputStream out = new FileOutputStream("/tmp/jrobin.png");
