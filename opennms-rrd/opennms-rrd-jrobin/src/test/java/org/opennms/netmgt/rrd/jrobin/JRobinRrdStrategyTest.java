...
public class JRobinRrdStrategyTest {
    ...
    public void testCommandWithoutDrawing() throws Exception {
        ...
        try {
            m_strategy.createGraph(command, Paths.get(""));
        } catch (Throwable t) {
    ...
    public void testDefWithEscapedCharacters() throws Exception {
        ...
        Throwable t = null;
        try {
            ((JRobinRrdStrategy) m_strategy).createGraphDef(Paths.get(""), command);
    ...
    public void testCreate() throws Exception {
        ...
        RrdDb openedFile = m_strategy.openFile(rrdFile.toAbsolutePath());
    ...
    public void testSampleVDEFPercentile() throws Exception {
        ...
        command = new String[] {
                "--start=" + (startTime - 300),
                "--end=" + (endTime + 300),
            "DEF:baz=" + rrdFile.toAbsolutePath().toString().replace("\\", "\\\\") + ":bar:AVERAGE",
            "VDEF:avg=baz,AVERAGE",                "VDEF:min=baz,MIN",
                "VDEF:max=baz,MAX",
                "VDEF:tot=baz,TOTAL",
                "VDEF:nfp=baz,95,PERCENT",
                "PRINT:avg:AVERAGE:\"%le\"",
        ...
        graphDef = ((JRobinRrdStrategy) m_strategy).createGraphDef(Paths.get(""), command);
        ...
        command = new String[] {
                "--start=" + (startTime - 300),
                "--end=" + (endTime + 300),
            "DEF:baz=" + rrdFile.toAbsolutePath().toString().replace("\\", "\\\\") + ":bar:AVERAGE",                "CDEF:bazX1=baz,1,*",
    ...
    public void testCommentWithNewlines() throws Exception {
        ...
        RrdGraphDef graphDef = ((JRobinRrdStrategy) m_strategy).createGraphDef(Paths.get(""), command);
        RrdGraph graph = new RrdGraph(graphDef);
        assertNotNull("graph object", graph);

        int firstHeight = graph.getRrdGraphInfo().getHeight();

        RrdGraphDef graphDef2 = ((JRobinRrdStrategy) m_strategy).createGraphDef(Paths.get(""), command2);
    ...
    public void testPrintThroughInterface() throws Exception {
        ...
        RrdGraphDetails graphDetails = m_strategy.createGraphReturnDetails(StringUtils.arrayToDelimitedString(command, " "), Paths.get(""));
    ...
    @Test
    public void testTWQnENoQNoEWithDEF() throws Exception {
    ...
    public void testFontArguments() throws Exception {
        ...
        JRobinRrdGraphDetails graphDetails = (JRobinRrdGraphDetails) m_strategy.createGraphReturnDetails(StringUtils.arrayToDelimitedString(command, " "), Paths.get(""));
        assertNotNull("graph details object", graphDetails);
    }
    ...
    public Path createRrdFile() throws Exception {
        System.err.println("createRrdFile()");
        String rrdFileBase = "foo";

        m_pathAnticipator.initialize();
        System.err.println("Got PathAnticipator");
        String rrdExtension = RrdUtils.getExtension();
        Path directory = m_pathAnticipator.getTempDir();
        System.err.println("temp dir: " + directory);
        ...
        RrdDef def = m_strategy.createDefinition("hello!", directory, rrdFileBase, 300, dataSources, rraList);
        System.err.println("Got RrdDef: " + def);
        m_strategy.createFile(def, null);
        System.err.println("Got RrdDb.");

        Path expecting = m_pathAnticipator.expecting(directory, rrdFileBase + rrdExtension);
        System.err.println("createRrdFile(): returning expecting: " + expecting);
        return expecting;
    }
}
