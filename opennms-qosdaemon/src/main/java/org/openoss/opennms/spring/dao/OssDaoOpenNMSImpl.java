...
public class OssDaoOpenNMSImpl implements OssDao {
     ...
     * (non-Javadoc)
     * @see
     * org.openoss.opennms.spring.dao.OssDao#setDataSource(javax.sql.DataSource)
     */
    /** {@inheritDoc} */
    @Override
     ...
     * (non-Javadoc)
     * @see
     * org.openoss.opennms.spring.dao.OssDao#setAssetRecordDao(org.opennms.netmgt
     * .dao.AssetRecordDao)
     */
    /** {@inheritDoc} */
    @Override
     ...
     * (non-Javadoc)
     * @see
     * org.openoss.opennms.spring.dao.OssDao#setNodeDao(org.opennms.netmgt.dao
     * .NodeDao)
     */
    /** {@inheritDoc} */
    @Override
     ...
     * (non-Javadoc)
     * @see
     * org.openoss.opennms.spring.dao.OssDao#setAlarmDao(org.opennms.netmgt.
     * dao.AlarmDao)
     */
    /** {@inheritDoc} */
    @Override
     ...
     * (non-Javadoc)
     * @see
     * org.openoss.opennms.spring.dao.OssDao#setTransTemplate(org.springframework
     ...
     * (non-Javadoc)
     * @see
     * org.openoss.opennms.spring.dao.OssDao#setQoSD(org.openoss.opennms.spring
     * .qosd.QoSD)
     */
    /** {@inheritDoc} */
    @Override
     ...
     * (non-Javadoc)
     * @see
     * org.openoss.opennms.spring.dao.OssDao#addCurrentAlarmForUniqueKey(org
     * .opennms.netmgt.model.OnmsAlarm)
     */
    ...
    public synchronized OnmsAlarm addCurrentAlarmForUniqueKey(final OnmsAlarm alarm) {
        ...
        if (checkAlarm != null) { // if not a unique alarm throw error
            throw new IllegalArgumentException(
                                               "OssDaoOpenNMSImpl().addCurrentAlarmForUniqueKey(): Illegal value: alarm not unique in Current Alarm list: ApplicationDN:"
            ...
            try { // add new alarm then save alarm in local alarm list
                ...
                alarmCacheByID.put(new Integer(alarm.getId()), alarm); // update
                                                                       // local
                                                                       // cache
            ...
            // alarm = getCurrentAlarmForUniqueKey(alarm.getApplicationDN() ,
            // alarm.getOssPrimaryKey());
            // } catch (Throwable ex){
        ...
        try {
            ...
            LOG.debug("OssDaoOpenNMSImpl().addCurrentAlarmForUniqueKey(): problem sending alarm to QoSD ( QoSD may not be running )",
                      e);
        }
        return alarm;
    }
     ...
     * (non-Javadoc)
     * @see
     * org.openoss.opennms.spring.dao.OssDao#updateCurrentAlarmForUniqueKey(
    ...
    public synchronized OnmsAlarm updateCurrentAlarmForUniqueKey(final OnmsAlarm alarm) {
        ...
        if (updateAlarm != null) { // if alarm in database then update alarm in
                                   // OpenNMS

            alarm.setId(updateAlarm.getId());
            try { // if alarm in database then update alarm in OpenNMS
                LOG.debug("\tOssDaoOpenNMSImpl().updateCurrentAlarmForUniqueKey: alarm to update in database",
                          alarmToStringBrief(alarm));
            ...
            throw new IllegalArgumentException(
                                               "OssDaoOpenNMSImpl().updateCurrentAlarmForUniqueKey(): Illegal value: alarm not found in Current Alarm list: ApplicationDN:"
                                                       + alarm.getApplicationDN()
                                                       + " OssPrimaryKey:"
                                                       + alarm.getOssPrimaryKey());
        }

        LOG.debug("OssDaoOpenNMSImpl().updateCurrentAlarmForUniqueKey: Sending Updated alarm list Updated alarm",
                  alarmToStringBrief(alarm));
     ...
     * (non-Javadoc)
     * @see
     * org.openoss.opennms.spring.dao.OssDao#getCurrentAlarmForUniqueKey(java
    ...
    private void localUpdateAlarmCacheTransaction() {
        ...
        for (int i = 0; i < alarms.length; i++) {
            ...
            // retrieve inner contents of alarm node if there is a node
            ...
            if (!((newalarm.getApplicationDN() == null) || (newalarm.getOssPrimaryKey() == null)
                    || (newalarm.getApplicationDN().equals("")) || (newalarm.getOssPrimaryKey().equals("")))) {
                ...
                if (alarmCacheByUniqueKey.get(uniqueKey) == null) {
                    ...
                    LOG.error("OssDaoOpenNMSImpl().localUpdateAlarmCache(): ERROR - duplicate alarm uniqueKey in database ={} AlarmID:{}",
                              uniqueKey, newalarm.getId());
                }
            }
     ...
     * (non-Javadoc)
     * @see
     * org.openoss.opennms.spring.dao.OssDao#updateAlarmCacheAndSendAlarms()
     */
    /**
     * <p>
     ...
     * @param ossPrimaryKey
     *            - unique OssPrimaryKey to search for
     * @return matching alarm if found or Null if match not found
     *         Not Thread Safe - only to be called from within the synchronised
     *         methods
     */
    private OnmsAlarm searchAlarmCacheForUniqueKey(String applicationDN, String ossPrimaryKey) {
        ...
        LOG.debug("OssDaoOpenNMSImpl().searchAlarmBufForUniqueKey alarmCacheByID search result:{}",
                  alarmToStringBrief(alarm));
    ...
    private static String alarmToStringBrief(OnmsAlarm alarm) {
        ...
        if (alarm == null) {
            ...
            s = "\n\t\tapplicationDN \t"
                    ...
                    + "\t\talarmID "
                    + alarm.getId() // alarmID
                    + "\t\tSeverity():" + alarm.getSeverity().getId() + "\t\tAlarmAckUser():" + alarm.getAlarmAckUser()
     ...
     * (non-Javadoc)
     * @see
     * org.openoss.opennms.spring.dao.OssDao#findNodeByLabel(java.lang.String)
     */
    /** {@inheritDoc} */
    @Override
     ...
     * (non-Javadoc)
     * @see
     * org.openoss.opennms.spring.dao.OssDao#findNodeByInstanceAndType(java.
     * lang.String, java.lang.String)
     */
    /** {@inheritDoc} */
    @Override
     ...
     * (non-Javadoc)
     * @see
     * org.openoss.opennms.spring.dao.OssDao#findNodeByID(java.lang.Integer)
     */
    /** {@inheritDoc} */
    @Override
        ...
        try {
                ...
                try {
                    ...
                    if (node.getLabel() != null) {
                        if (nodeCacheByLabel.get((String) node.getLabel()) != null) {
                            LOG.info("OssDaoOpenNMSImpl().updateNodeCaches WARNING node.getId():{} Node Label:{} is duplicated",
                                     node.getId(), node.getId());
                        } else {
                            nodeCacheByLabel.put((String) node.getLabel(), node);
                        }
                    } else {
                        LOG.info("OssDaoOpenNMSImpl().updateNodeCaches WARNING node.getId():{} Node Label is NULL. Not putting node in nodeCacheByLabel",
                                 node.getId());
                    }

                    // update node by Unique ID
                    // -managedObjectInstance+ManagedObjectType
                    final OnmsAssetRecord assetRecord = node.getAssetRecord();
                    if (assetRecord == null) {
                        LOG.info("OssDaoOpenNMSImpl().updateNodeCaches WARNING node.getId():{} assetRecord is NULL. Not putting node in nodeCacheByUniqueID",
                                 node.getId());
                        continue;
                    } else {

                        // Note that the node asset record data for instance and
                        // type are only filled
                        // given default values once - subsequently changes must
                        // be explicitly set directly
                        // in the database
                        ...
                        if ((moi == null) || ("".equals(moi))) {
                            ...
                            LOG.info("OssDaoOpenNMSImpl().updateNodeCaches WARNING node.getId():{} ManagedObjectInstance is NULL. Setting  ManagedObjectInstance to: {}",
                                     node.getId(), moi);
                        ...
                        if ((mot == null) || ("".equals(mot))) {
                            ...
                            LOG.info("OssDaoOpenNMSImpl().updateNodeCaches WARNING node.getId():{} ManagedObjectType was NULL. Setting ManagedObjectType to: {}",
                                     node.getId(), mot);
                        ...
                        if (nodeCacheByUniqueID.get((String) uniqueid) != null) {
                            LOG.info("OssDaoOpenNMSImpl().updateNodeCaches WARNING node.getId():{} Unique ID is duplicated. Unique ID = ManagedObjectInstance:{} ManagedObjectType:{}",
                                     node.getId(), assetRecord.getManagedObjectInstance(),
            ...
            if (LOG.isDebugEnabled()) {
                ...
                try {
                    ...
                    LOG.error("OssDaoOpenNMSImpl().updateNodeCaches: Problem listing nodeCacheByLabel contents Error:",
                              e);
                }
                ...
                try {
                    ...
                    LOG.error("OssDaoOpenNMSImpl().updateNodeCaches: Problem listing nodeCacheByUniqueID contents Error:",
                              e);
                }
            }
        } catch (Throwable ex) {
