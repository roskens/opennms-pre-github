...
public interface OssDao {
    ...
    /**
     * Adds Current alarm to OpenNMS database with a new alarmID as an
     * AlarmType= 'raise' ( type 1 ) alarm.
     * Adds the alarm to the local Current Alarm Alarm list alarmCacheByID with
     * the new alarmID only if
    ...
    /**
     * Updates Current alarm in OpenNMS database with a new alarmID as an
     * AlarmType= 'raise' ( type 1 ) alarm.
     * Adds the alarm to the local Current Alarm Alarm list alarmCacheByID with
     * the new alarmID only if
     * the alarm is NOT (Acknowledged AND Cleared).
    ...
    /**
     * Used By QoSD to force an update to the local cache from latest alarm list
     * in database
     * Tries to call back to QoSD to send the latest alarm list.
     * The reason for this is to enforce synchronisation between QoSD and QoSDrx
     * so that the
     ...
     * Note NOT Synchronized - but OK if called by QoSD through
     * QoSD.sendAlarms()
     *
    ...
    /**
     * This will return the first node in nodes table with nodeLable entry
     * matching label
     ...
     * @return will look for first match of node label. <code>null</code> if not
     *         found
    ...
    /**
     * This will return the first node with entry in Assets table having
     * matching managedObjectInstance and
     * managedObjectType.
     * Note for this to work, the configuration of OpenNMS must ensure that the
     * concatenation of
     * these fields is unique in the system otherwise only the first instance
     * will be returned
     *
