...
/**
 * Wrapper class for OssDaoOpenNMSImpl which makes it onto a singleton which can
 * be shared
 * between Qosd and QoSDrx when either one or or both applications are running.
 * This is needed because Qosd and
 * QoSDrx have different local application contexts but need to share access to
 * the OssDao which provides the
 * synchronized alarm cache for both applications.
 * Either Qosd or QoSDrx can initialise the class. The first call to
 * getInstance() causes the class to be created.
 * All subsequent calls return the same instance.
 * Note it is expected that the Spring application context has already set up
 * the opennms DAO's before the first
 * call to getInstance(). This means that you must ensure that the application
 * contexts for Qosd and QosDrx
 * set the same values for these DAO's otherwise there will be unpredictable
 * results
 *
 * @author ranger
 * @version $Id: $
...
public class OssDaoOpenNMSImplSingleton {
    ...
    public static OssDao getInstance() {
                              ...
                              // it
        {
            ...
            synchronized (OssDaoOpenNMSImplSingleton.class) {
                ...
                // Seth 2010-05-04: This field was being set on the singleton
                // factory,
                // but not the constructed instances; keeping it that way for
                // now
    ...
    /**
     * <p>
     * Setter for the field <code>alarmDao</code>.
     * </p>
     *
     * @param alarmDao
    ...
    /**
     * <p>
     * Setter for the field <code>assetRecordDao</code>.
     * </p>
     *
    ...
    /**
     * <p>
     * Setter for the field <code>transTemplate</code>.
     * </p>
     *
     * @param transTemplate
     *            a
