 ...
 * updates to
 * the Node list and the Alarm List in the ossDao. If OpenNMS issues multiple
 * events to the QoSD
 * event handler which cause it to want to update the alarm list or update the
 * node list, this Thread latches
 * the event and will request the ossDao to update it's cache and send any new
 * alarms.
 * If further events occur while the ossDao is updating, these will be latched
 * until the
 * process completes. This prevents every new opennms alarm causing a call to
 * the ossDao while it is running
 * which would otherwise back up a queue of requests against it's synchronized
 * methods.
 *
 * @author ranger
 * @version $Id: $
...
public class OpenNMSEventHandlerThread extends Thread {
    ...
    /**
     * If called, this method will request that the ossDao Alarm Cache is
     * updated from the OpenNMS database
     * and sent to the QoSD for processing.
     * Note that multiple calls while the update process is running will be
    ...
    /**
     * If called, this method will request that the ossDao Node Cache is updated
     * from the OpenNMS database
     * and sent to the QoSD for processing.
     * Note that multiple calls while the update process is running will be
     * latched ( i.e. not queued) and result
