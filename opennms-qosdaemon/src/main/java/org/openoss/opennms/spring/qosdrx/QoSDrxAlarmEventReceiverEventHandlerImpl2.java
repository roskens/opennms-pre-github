...
public class QoSDrxAlarmEventReceiverEventHandlerImpl2 implements AlarmEventReceiverEventHandler {
     ...
     * the receiver name will be used as the node name which will be updated
     * with
     ...
     * the alarm will be created with the node name corrsponding to a
     * concatenation
    ...
    /*
     * determines the alarm update behaviour. Must be set to
     * <code>SPECIFY_OUTSTATION</code> or
     * <code>USE_TYPE_INSTANCE</code>
     */
    /** The alm update behaviour. */
     ...
     */
    /** The alarm update behaviour. */
    private String alarmUpdateBehaviour = null;

    /**
     * Used by Spring Application context to pass in alarmUpdateBehaviour as a
     * string.
     *
     * @param _alarmUpdateBehaviour
     *            must be <code>"USE_TYPE_INSTANCE"</code> or
     *            <code>"SPECIFY_OUTSTATION"</code>
     */
    ...
    // TODO remove
    // private static boolean ossDaoIsInitialised=false; // TODO - may want this
    // in the spring initialisation
    ...
    public synchronized void init() {
        ...
        try {
            ...
            throw new UndeclaredThrowableException(ex, this.getClass().getSimpleName()
                    + "init() problem initialising class");
        }
        ...
        // this.getClass().getSimpleName()+"init() problem initialising class");
        // }
        // }
        // initialised=true;
    }
    ...
    public void onNotifyNewAlarmEvent(NotifyNewAlarmEvent nnae, OssBeanAlarmEventReceiver callingAer) {
        ...
        String logheader = "RX:" + callingAer.getName() + ":" + this.getClass().getSimpleName()
                + ".onNotifyNewAlarmEvent(): ";

        LOG.debug("{} Statistics: {}", logheader, callingAer.getRuntimeStatistics());
        if (!initialised) {
            LOG.error("{} event handler not initialised. init() must be called by receiver before handling any events",
                      logheader);
            return;
        }
        ...
        try {
            ...
            LOG.debug("{} Received an onNotifyNewAlarmEvent() - AlarmPrimaryKey: {} ApplictionDN: {} eventtime: {}",
                      logheader, ossPrimaryKey, applicationDN, nnae.getEventTime());
            LOG.debug("{} Using this OssDao (toString):{}", logheader, ossDao.toString());
            if ((applicationDN == null) || (applicationDN.equals("")) || (ossPrimaryKey == null)
                    || (ossPrimaryKey.equals(""))) {
                LOG.error("{} ApplicatioDN or PrimaryKey not set", logheader);
            } else {
                LOG.debug("{} Creating new alarm", logheader);

                // alarm = ossDao.getCurrentAlarmForUniqueKey(applicationDN,
                // ossPrimaryKey);
                // if (alarm!=null) { // already an alarm with this unique id -
                // LOG.error
                // LOG.error("{} Alarm Already exists with this Unique ID",
                // logheader);
                // } else {
                alarm = new OnmsAlarm();
                ...
                alarm.setReductionKey(":managedObjectInstance:" + nnae.getManagedObjectInstance()
                        + ":managedObjectType:" + nnae.getManagedObjectClass() + ":ossPrimaryKey:-" + ossPrimaryKey
                        + ":applicationDN:-" + applicationDN); // must be unique
                                                               // because of
                ...
                OnmsNode node = new OnmsNode(); // TODO remove
                                                // ossDao.makeExtendedOnmsNode();
                node.setId(new Integer(1)); // node id cannot be null
                alarm.setNode(node); //

                if (almUpdateBehaviour == null) {
                    LOG.error("RX:{}: This receiver's alarmUpdateBehaviour is not set: defaulting to update nodeID:1",
                              callingAer.getName());
                } else if (callingAer.getName() == null) {
                    LOG.error("RX:{}: This receiver has no name: default alarms will update nodeID:1",
                              callingAer.getName());
                } else {
                    ...
                    if (almUpdateBehaviour.equals(SPECIFY_OUTSTATION)) {
                        // this will look for first match of node label to
                        // callingAer.getName()
                        // and set node id to this value.

                        LOG.debug("{} SPECIFY_OUTSTATION looking for node with nodelabel:{}", logheader,
                                  callingAer.getName());
                        try {
                            // TODO temp remove ?
                            try {
                                ...
                                LOG.error("{} alarmUpdateBehaviour.equals(USE_TYPE_INSTANCE) Problem looking up Node ",
                                          logheader, ex);
                            }

                            if (node != null) {
                                LOG.debug("{} alarmUpdateBehaviour.equals(SPECIFY_OUTSTATION):NODE FOUND for this RX Name:{} setting node id to NodeLabel:{} NodeID:{}",
                                          logheader, callingAer.getName(), node.getLabel(), node.getId());
                                alarm.setNode(node); // maps into FIRST instance
                                                     // of node with the same
                                                     // managedObjectInstance
                                                     // and managedObjectType
                            } else {
                                LOG.error("{} alarmUpdateBehaviour.equals(SPECIFY_OUTSTATION):NODE NOT FOUND for this RX Name:{} setting node id to default NodeID: 1",
                                          logheader, callingAer.getName());
                                node = new OnmsNode(); // TODO remove
                                                       // ossDao.makeExtendedOnmsNode();
                                node.setId(new Integer(1)); // node id cannot be
                                                            // null
                                alarm.setNode(node); //
                            }
                        } catch (Throwable ex) {
                            LOG.error("{} alarmUpdateBehaviour.equals(USE_TYPE_INSTANCE) Problem looking up Node for alarm Set to default nodeID:1",
                                      logheader, ex);
                        }

                    } else if (almUpdateBehaviour.equals(USE_TYPE_INSTANCE)) {
                        // this will look for first match of node Managed object
                        // Instance and Managed Object type
                        // and set node id to this value.
                        String managedObjectType = nnae.getManagedObjectClass();
                        String managedObjectInstance = nnae.getManagedObjectInstance();

                        LOG.debug("{} USE_TYPE_INSTANCE looking for node with managedObjectType:{} managedObjectInstance:{}",
                                  logheader, managedObjectType, managedObjectInstance);
                        try {
                            node = ossDao.findNodeByInstanceAndType(managedObjectInstance, managedObjectType);

                            if (node != null) {
                                LOG.debug("{} alarmUpdateBehaviour.equals(USE_TYPE_INSTANCE):NODE FOUND for this RX Name:{} setting node id to NodeLabel:{} NodeID:{}",
                                          logheader, callingAer.getName(), node.getLabel(), node.getId());
                                alarm.setNode(node); // maps into FIRST instance
                                                     // of node with the same
                                                     // managedObjectInstance
                                ...
                                LOG.error("{} alarmUpdateBehaviour.equals(USE_TYPE_INSTANCE):NODE NOT FOUND for this managedObjectType:{} managedObjectInstance:{} setting node id to default NodeID: 1",
                                          logheader, managedObjectType, managedObjectInstance);
                                node = new OnmsNode(); // TODO remove
                                                       // ossDao.makeExtendedOnmsNode();
                                node.setId(new Integer(1)); // node id cannot be
                        ...
                        LOG.error("{} Invalid value for alarmUpdateBehaviour:{} {} defaulting to update nodeID:1",
                                  logheader, almUpdateBehaviour, alarmUpdateBehaviour);
                    }
                }
                ...
                // //event.setId(new Integer(1)); // This is NOT set since
                // unique constraint in alarms table on Events table
                ...
                // alarm.setDistPoller(new
                // OnmsDistPoller("undefined","localhost")); //simple
                // constructor
                alarm.setDistPoller(distPollerDao.get("localhost"));

                alarm.setDescription(nnae.getAdditionalText()); // TODO need
                                                                // Qosd Not to
                                                                // generate this
                                                                // if remote
                ...
                try {
                    ...
                    LOG.error("{} problem creating new alarm AlarmPrimaryKey: {} ApplictionDN: {}", logheader,
                              ossPrimaryKey, applicationDN, ex);
                }
            }
            // TODO remove }
        } catch (Throwable e) {
    ...
    public void onNotifyClearedAlarmEvent(NotifyClearedAlarmEvent nclae, OssBeanAlarmEventReceiver callingAer) {
        ...
        try {
            ...
            LOG.debug("{} Received an onNotifyClearedAlarmEvent() - AlarmPrimaryKey: {} ApplictionDN: {} eventtime: {}",
                      logheader, ossPrimaryKey, applicationDN, nclae.getEventTime());
            if ((applicationDN == null) || (applicationDN.equals("")) || (ossPrimaryKey == null)
                    || (ossPrimaryKey.equals(""))) {
                LOG.error("{} ApplicatioDN or PrimaryKey not set", logheader);
            } else {
                ...
                if (alarm == null) { // no alarm with this unique id - LOG.error
                    LOG.info("{} WARNING Alarm does not exist with this Unique ID:- AlarmPrimaryKey: {} ApplictionDN: {}",
                             logheader, ossPrimaryKey, applicationDN);
                } else {
                    ...
                    alarm.setSeverity(OnmsSeverity.CLEARED); // TODO need
                                                             // mapping for
                                                             // severity
                    ...
                    // alarm.setNode(arg0); // TODO mapping to node if in
                    // database
                    // alarm.setMouseOverText(arg0);
                    // alarm.setManagedObjectType(nclae.getManagedObjectClass());
                    // // TODO check if changed
                    // alarm.setManagedObjectInstance(nclae.getManagedObjectInstance());
                    // //TODO check if changed
                    // alarm.setLogMsg(arg0);

                    // TODO REMOVED - DO NOT CREATE EVENT WITH HIBERNATE
                    // AlarmDAo
                    // OnmsEvent event= new OnmsEvent();
                    // //event.setId(new Integer(1)); // This is NOT set since
                    // unique constraint in alarms table on Events table
                    ...
                    alarm.setLastEventTime(nclae.getEventTime()); // must be
                                                                  // after
                                                                  // .setLastEvent!!!
                    ...
                    alarm.setAlarmAckUser("ossjclearevent"); // TODO CLEARING
                                                             // ALARMS ON
                                                             // RECEIPT OF CLEAR
                                                             // - NOT WAITING
                                                             // FOR ACK
                    ...
                    try {
                        ...
                        LOG.error("{} problem clearing new alarm AlarmPrimaryKey: {} ApplictionDN: {}", logheader,
                                  ossPrimaryKey, applicationDN, ex);
                    }
                }
            }
    ...
    public void onNotifyAckStateChangedEvent(NotifyAckStateChangedEvent nasce, OssBeanAlarmEventReceiver callingAer) {
         ...
         * try{
         * LOG.debug(
         * "QoSDrxAlarmEventReceiverEventHandlerImpl().onNotifyAckStateChangedEvent(): Received an NotifyAckStateChangedEvent - AlarmPrimaryKey: {} New Ack State: {}"
         * , nasce.getAlarmKey().getAlarmPrimaryKey(),
         * nasce.getAlarmAckState());
         * OnmsAlarm alarm=null;
         * try {
         ...
         * LOG.error(
         * "QoSDrxAlarmEventReceiverEventHandlerImpl().onNotifyAckStateChangedEvent():: nasce alarm key set in illegal state"
         * , ise);
         * }
         * catch (java.lang.IllegalArgumentException iae){
         * LOG.error(
         * "QoSDrxAlarmEventReceiverEventHandlerImpl().onNotifyAckStateChangedEvent(): {}"
         * , iae);
         * }
         * if(nasce.getAlarmAckState() == AlarmAckState.ACKNOWLEDGED) {
         * if (alarm != null ) { // if opennms has an alarm with this id to
         * update
         * try {
         ...
         * LOG.debug(
         * "QoSDrxAlarmEventReceiverEventHandlerImpl().onNotifyAckStateChangedEvent(): Acknowledging Alarm: {} New Ack State: {}"
         * , nasce.getAlarmKey().getAlarmPrimaryKey(),
         * nasce.getAlarmAckState());
         * }
         * catch ( Exception ex ) {
         * LOG.error(
         * "QoSDrxAlarmEventReceiverEventHandlerImpl().onNotifyAckStateChangedEvent():: problem updating alarm ack state"
         * , ex);
         * }
         * }
         * else { //if opennms does not have an alarm with this id to update
         * LOG.debug(
         * "QoSDrxAlarmEventReceiverEventHandlerImpl().onNotifyAckStateChangedEvent(): Alarm cannot be acknowledged - not in database: {}"
         * , nasce.getAlarmKey().getAlarmPrimaryKey());
         * }
         ...
         * ossDao.updateCurrentAlarmForUniqueKey(alarm);
         * LOG.debug(
         * "QoSDrxAlarmEventReceiverEventHandlerImpl().onNotifyAckStateChangedEvent(): UnAcknowledging Alarm: {} New Ack State: {}"
         * , nasce.getAlarmKey().getAlarmPrimaryKey(),
         * nasce.getAlarmAckState());
         * }
         * catch ( Exception ex ) {
         ...
         * else { //if opennms does not have an alarm with this id to update
         * LOG.debug(
         * "QoSDrxAlarmEventReceiverEventHandlerImpl().onNotifyAckStateChangedEvent(): Alarm cannot be Unacknowledged - not in database: {}"
         * , nasce.getAlarmKey().getAlarmPrimaryKey());
         * }
         * }
         * }
         * catch(Throwable e){
         * LOG.error(
         * "QoSDrxAlarmEventReceiverEventHandlerImpl().onNotifyAckStateChangedEvent() Error : "
         * , e);
         * }
         */
