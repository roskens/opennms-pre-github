...
public class OnmsAlarmOssjMapper {
    ...
    /** The alarm update behaviour str. */
     ...
     *
     * @param aub
     *            valid value for <code>alarmUpdateBehaviour</code>
     *            SPECIFY_OUTSTATION, or USE_TYPE_INSTANCE
     * @return the alarm update behaviour for int
     */
     ...
     *
     * @param onmsAlarm
     *            OnmsAlarm object to be populated
     * @param alarmValue
     *            OSS/J AlarmValue data to use to populate OnmsAlarm
     * @param almUpdateBehaviour
     *            - determines how to treat the node name of the new alarm must
     *            be of value; <code>USE_TYPE_INSTANCE</code> - populate nodeID
     *            with node having same asset type and instance data as alarm
     *            or <code>SPECIFY_OUTSTATION</code> - populate nodeID with node
     *            having same nodeLabel as defaultUpdateNodeLabel
     * @param defaultUpdateNodeLabel
     *            name of node to be updated if
     ...
     */
    public OnmsAlarm populateOnmsAlarmFromOssjAlarm(OnmsAlarm onmsAlarm, AlarmValue alarmValue,
            Integer almUpdateBehaviour, String defaultUpdateNodeLabel) throws IllegalArgumentException,
        ...
        try {
            ...
            LOG.debug("{} AlarmPrimaryKey: {} ApplictionDN: {} alarmRaisedTime: {}", logheader, ossPrimaryKey,
                      applicationDN, alarmValue.getAlarmRaisedTime());
            if ((applicationDN == null) || (applicationDN.equals("")) || (ossPrimaryKey == null)
                ...
                if (onmsAlarm != null) { // already an alarm with this unique id
                                         // - log error
                    ...
                    onmsAlarm.setReductionKey(":managedObjectInstance:" + alarmValue.getManagedObjectInstance()
                            + ":managedObjectType:" + alarmValue.getManagedObjectClass() + ":ossPrimaryKey:-"
                            + ossPrimaryKey + ":applicationDN:-" + applicationDN); // must
                                                                                   // be
                                                                                   // unique
                                                                                   // because
                                                                                   // of
                    ...
                    if (almUpdateBehaviour == null) {
                        LOG.error("{} This receiver's alarmUpdateBehaviour is not set: defaulting to update nodeID:1",
                                  logheader);
                    } else {
                        LOG.debug("{} alarmUpdateBehaviour:{} {}", logheader, almUpdateBehaviour,
                        ...
                        if (almUpdateBehaviour.equals(SPECIFY_OUTSTATION)) {
                            ...
                            try {
                                ...
                                if (node != null) {
                                    LOG.debug("{} alarmUpdateBehaviour.equals(SPECIFY_OUTSTATION): NODE FOUND for this name:{} setting node id to NodeLabel:{} NodeID:{}",
                                              logheader, defaultUpdateNodeLabel, node.getLabel(), node.getId());
                                    onmsAlarm.setNode(node); // maps into FIRST
                                                             // instance of node
                                                             // with the same
                                    ...
                                    LOG.error("{} alarmUpdateBehaviour.equals(SPECIFY_OUTSTATION): NODE NOT FOUND for this name:{} setting node id to default NodeID: 1",
                                              logheader, defaultUpdateNodeLabel);
                                    node = new OnmsNode(); // TODO remove
                                                           // ossDao.makeExtendedOnmsNode();
                                    node.setId(new Integer(1)); // node id
                                                                // cannot be
                                                                // null
                                    onmsAlarm.setNode(node); //
                                }
                            ...
                            // this will look for first match of node Managed
                            // object Instance and Managed Object type
                            // and set node id to this value.
                            ...
                            LOG.error("{} Invalid value for alarmUpdateBehaviour:{} {} defaulting to update nodeID:1",
                                      logheader, almUpdateBehaviour, getAlarmUpdateBehaviourForInt(almUpdateBehaviour));
                        }
                    }
                    ...
                    // NOTE - this has no effect here as .setLastEvent nulls
                    // value
                    ...
                    onmsAlarm.setAlarmType(new Integer(1)); // set to raise
                                                            // alarm
    ...
    /**
     * This method maps OpenNMS alarm to an OSS/J alarms and adds additional
     * information.
     *
     * @param alarmValueSpecification
     *            AlarmValue object to be populated - Invariant (Specifcation)
     *            values should be already populated
     * @param _openNMSalarm
     *            data to use to populate the OSS/J alarm
     ...
     *             the unsupported attribute exception
     */
    public AlarmValue populateOssjAlarmFromOpenNMSAlarm(AlarmValue alarmValueSpecification, OnmsAlarm _openNMSalarm)
        ...
        // test to see if opennms alarm already has type and instance
        // information set. If yes then it has most likely
        // come from Qosdrx.
        if ((_openNMSalarm.getManagedObjectInstance() != null) && (_openNMSalarm.getManagedObjectType() != null)
            ...
            isQoSDrxAlarm = true;
            LOG.debug("{} isQoSDrxAlarm TRUE - because OpenNMS alarm has ManagedObjectInstance and ManagedObjectType",
                      logheader);
        } else {
            isQoSDrxAlarm = false;
            LOG.debug("{} isQoSDrxAlarm FALSE - because OpenNMS alarm NOT POPULATED ManagedObjectInstance and ManagedObjectType",
                      logheader);
        }

        try {
            // If the alarm has both an acknowledge time and an acknowledge user
            // then the alarm has been acknowledged. Set the corrsponding
            // parameters
            // in the OSS/J alarm
        ...
        try {
            ...
            if (null != uei) {
                ...
                _uei_no_html = matcher.replaceAll(" "); // remove any HTML tags
                                                        // from uei
            }
            alarmValueSpecification.setAlarmType((_openNMSalarm.getX733AlarmType() == null) ? javax.oss.fm.monitor.AlarmType.EQUIPMENT_ALARM
        ...
        if (!isQoSDrxAlarm) { // if is locally generated alarm
            try {
                ...
                if (_openNMSalarm.getNode() != null) {
                    ...
                    // }
                    if (asset != null) {
                        // if (asset.getManufacturer()!= null) mftr =
                        // asset.getManufacturer(); // Not read
                        // if (asset.getModelNumber()!= null) modelNo =
                        // asset.getModelNumber(); // Not read
                    ...
                    LOG.debug("{} isQoSDrxAlarm=FALSE  OpenNMS type and instance not set. Using from Node Asset record: ManagedObjectInstance: {} ManagedObjectType:{}",
                              logheader, managedObjectInstance, managedObjectType);
                }
            } catch (Throwable ex) {
        ...
        try {
            ...
            // alarmValueSpecification.setProbableCause((short)-1); // OSS/J set
            // to -1 then text contains description
        ...
        if (!isQoSDrxAlarm) { // if is a locally generated alarm
            ...
            try {
                ...
                // note some OnmsAlarms can have null nodes - we use a default
                // value of 0 for ID
                Integer nodeid = 0;
                ...
                alarmValueSpecification.setAdditionalText("<alarmid>" + alarmid + "</alarmid>" + "\n            "
                        ...
                        + InetAddressUtils.toIpAddrString(ipaddress) + "</ipaddress>" + "\n            "
                        + "<description>" + _description + "</description>" + "\n            " + "<opinstr>" + _opinstr
                        + "</opinstr>" + "\n            " + "<asset.managedobjectinstance>"
                        + assetManagedObjectInstance + "</asset.managedobjectinstance>"
                        + "\n            "
        ...
        try {
            // populate alarm key
            // TODO was AlarmKey ak = new
            // OOSSAlarmKey(Integer.toString(_openNMSalarm.getId()));
     ...
     * NOTE org.opennms.web.alarm.Alarm.NORMAL_SEVERITY has no equivilent in
     * OSS/J X733
     *
     * @param ossjseverity
     ...
     *
     * @param alarmType
     *            String representing OSS/J alarm Type
