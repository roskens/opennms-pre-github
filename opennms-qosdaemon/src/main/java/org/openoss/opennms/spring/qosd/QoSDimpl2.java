 ...
 * The start() method loads the configuration for the QosD daemon and registers
 * for events
 ...
 * <p>
 * 1. When the deamon starts it initialises the
 * <CODE>AlarmListJ2eeConnectionManagerThread</CODE> and registers with the
 * AlarmMonitor bean in the application server.
 * <p>
 * 2. It then calls the AlarmListJ2eeConnectionManagerThread.Reset_List in order
 * to cause the interface to send an AlarmListRebuiltEvent.
 * <p>
 * The JNDI naming factory, JMS queues and ejb's conected to by the daemon are
 * specified in the qosd.properties file. The location of qosd.properties file
 * is set by the JRE system variable -DpropertiesFile which should be set when
 * OpenNMS is started up. This is set in /etc/opennms.conf file Contents of
 * opennms.conf: <CODE>
 ...
 * </CODE> rmi.policy sets
 * the security settings to allow the JVM to connect externally Contents of
 * rmi.policy: <CODE>grant{permission java.security.AllPermission;};</CODE>
 * <p>
 * 3. The daemon then sends out the full current alarm list to the AlarmMonitor
 * bean and registers with OpenNMS for events
 * <p>
 * The events used to run the QosD bean are determined by the file
 * /etc/QoSD-configuration.xml By default only the
 * 'uei.opennms.org/vacuumd/alarmListChanged' uei is included in this file. This
 * event is generated when the <code>notifyOSSJnewAlarm</code> automation
 * running in the vacuumd deamon determines that the alarm list has changed. In
 * normal operation there is a short delay between an alarm entering the alarm
 * list and the notifyOSSJnewAlarm automation picking it up. This can be
 * significantly shortend for high priority alarms if their raise uei's are also
 * included in the QoSD-configuration.xml file. However for most alarms this is
...
public class QoSDimpl2 extends AbstractServiceDaemon implements EventListener, QoSD {
    ...
    /**
     * {@inheritDoc} provides an interface to OpenNMS which provides a unified
     * api
     */
    @Override
    ...
    /**
     * {@inheritDoc} Used by Spring Application context to pass in
     * OnmsAlarmOssjMapper
    ...
    /**
     * {@inheritDoc} Used by Spring Application context to pass in NodeDaof
     */
    @Override
    ...
    /**
     * {@inheritDoc} Used by Spring Application context to pass in alarmDao
     */
    @Override
    ...
    protected void onStart() {
        ...
        try {
            LOG.debug("Qosd.start():setting application context for alarmListConnectionManager: m.context.toString:{}",
                      m_context.toString());
         ...
         * exists in
         * the properties file. If it does then it stores it in the jnp_host
         * string. If
         * it doesn't then it uses a default naming provider string
         * "jbossjmsserver1:1099" and
        ...
        try {
            ...
            // send alarm list rebuilt event to EJB via the connection manager
            // thread.
            alarmListConnectionManager.reset_list("openNMS just restarted - alarm list rebuilt. Time:" + new Date()); // send
                                                                                                                      // an
                                                                                                                      // alarm
                                                                                                                      // list
                                                                                                                      // rebuilt
        ...
        try {
            // this.sendAlarms(); // interface has just started up. Send all
            // alarms
    ...
    /**
     * {@inheritDoc} The OpenNMS event listener runs this routine when a
     * new event is detected. This can be run on any event but only needs to run
     * on
     * uei.opennms.org/vacuumd/alarmListChanged
     */
    @Override
    public void onEvent(Event event) {
        LOG.debug("Qosd.onEvent: OpenNMS Event Detected by QosD. uei '{}' Dbid(): {}  event.getTime(): {}",
        ...
        if (event.getUei().equals("uei.opennms.org/vacuumd/alarmListChanged")) {
            ...
            // used code from AlarmWriter.java Check value of <logmsg> attribute
            // 'dest', if set to
            // "donotpersist" then simply return, the uei is not to be persisted
            // to the database
            // The uei.opennms.org/vacuumd/alarmListChanged event must be set to
            // be reised
            // as auto-action from vacumd-configuration.xml and is called
            // when vacumd updates the current ( alarm not acknowledgd and
            // cleared ) alarm list
            // note alarmListChanged event may be marked as 'donotpersist' so
            // checked first
            try {
                ...
                // AlarmData should not be null if QoSD-configuration.xml is set
                // up only to receive raise
                // and not clearing alarms
                if (event.getAlarmData().getAlarmType() == 2) {
                    LOG.debug("Qosd.onEvent: uei '{}' Dbid(): {} alarm type = 2 (clearing alarm) so ignoring.",
                              event.getUei(), event.getDbid());
                    return;
                }
            } catch (NullPointerException e) {
                LOG.error("Qosd.onEvent: uei '{}' Dbid(): {}' problem dealing with event. Check QoSD-configuration.xml.",
                          event.getUei(), event.getDbid());
                return;
            }
        }

        // This forces the ossDao to update it's list on this event and call
        // back to sendAlarms() to send the
        // updated alarm list.
        try {
    ...
    /**
     * A method to request an alarm list from the OpenNMS database using the
     * ossDao,
     * convert them to OSS/J alarms using the onmsAlarmOssjMapper and send the
     * OSS/J alarms
     * using the alarm list connection manager (alcm) to update the the
     * AlarmMonitor bean.
    ...
    public void sendAlarms() {
        ...
        try {
            ...
            throw new UndeclaredThrowableException(ex,
                                                   "sendAlarms() Cannot retrieve alarms from ossDao.getAlarmCache()");
        }

        LOG.debug("sendAlarms() Alarms fetched. Processing each alarm in list.");
        // Convert the OnmsAlarm array alarmBuf to a hashtable using the alarmid
        // as the key.
        try {
            for (int i = 0; i < onmsAlarmUpdateList.length; i++) {
                LOG.debug("sendAlarms() processing an OpenNMS alarm:");

                // if useUeiList is true only the alarms whose UEI's are listed
                // in the
                ...
                if (useUeiList) {
                    ...
                    if (null == triggerUeiList.get(onmsAlarmUpdateList[i].getUei())) {
                        LOG.debug("sendAlarms() alarm UEI not in QosD-configuration.xml. Not sending. alarmID:{} alarmUEI:{}",
                                  onmsAlarmUpdateList[i].getId(), onmsAlarmUpdateList[i].getUei());
                        continue; // ignore this event and return
                    }
                    LOG.debug("sendAlarms() alarm UEI is in QosD-configuration.xml. Trying to send alarmID:{} alarmUEI:{}",
                              onmsAlarmUpdateList[i].getId(), onmsAlarmUpdateList[i].getUei());
                }

                if (onmsAlarmUpdateList[i].getAlarmType() != 1) {
                    LOG.debug("sendAlarms() Alarm AlarmType !=1 ( not raise alarm ) Not sending alarmID:{} :alarmBuf[i].getQosAlarmState()=: {}",
                              onmsAlarmUpdateList[i].getId(), onmsAlarmUpdateList[i].getQosAlarmState());
                    continue;
                } else {
                    LOG.debug("sendAlarms() Alarm AlarmType==1 ( raise alarm ) Sending alarmID:{} :alarmBuf[i].getQosAlarmState()=: {}",
                              onmsAlarmUpdateList[i].getId(), onmsAlarmUpdateList[i].getQosAlarmState());
                    try {

                        // Code which creates the OSSJ AlarmValue from the
                        // Spring OSS?J AlarmValue Specification
                        LOG.debug("sendAlarms(): generating the OSS/J alarm specification:");
                        ossjAlarm = alarmListConnectionManager.makeAlarmValueFromSpec();
                        LOG.debug("sendAlarms(): OSS/J alarm specification:{}",
                                  OOSSAlarmValue.converttoString(ossjAlarm));

                        // Code which creates the OSSJ AlarmValue from the
                        // Spring OSS/J AlarmValue Specification
                        LOG.debug("sendAlarms(): onmsAlarmOssjMapper.populateOssjAlarmFromOpenNMSAlarm:");
                        ossjAlarm = onmsAlarmOssjMapper.populateOssjAlarmFromOpenNMSAlarm(ossjAlarm,
                                                                                          onmsAlarmUpdateList[i]);
                        ...
                        /*
                         * else try {
                         * // no alarms which are ACKNOWLEDGED and CLEARED are
                         * included in this current alarm list
                         * if (ossjAlarm.getAlarmAckState() ==
                         * javax.oss.fm.monitor.AlarmAckState.UNACKNOWLEDGED ) {
                         * ossjAlarmUpdateList.put(ossjAlarm.getAlarmKey(),
                         * ossjAlarm); // put all unacknowledged alarms in list
                         * }
                         * else if (ossjAlarm.getPerceivedSeverity() !=
                         * javax.oss.fm.monitor.PerceivedSeverity.CLEARED ){
                         * ossjAlarmUpdateList.put(ossjAlarm.getAlarmKey(),
                         * ossjAlarm); // put all uncleared acknowledged alarms
                         * in list
                         * }
                         * } catch (Throwable e) {
                         * log.error(
                         * "sendAlarms() error in alarmACKState or PercievedSeverity - check alarm definitons"
                         * , e);
                         * }
                         */
                    } catch (Throwable ex) {
                        LOG.error("sendAlarms() error trying to populate alarm - alarm disguarded - check alarm definitons",
                                  ex);
                    }
                }
            }
        ...
        try {
            ...
            if (LOG.isDebugEnabled()) {
                ...
                for (Entry<AlarmKey, AlarmValue> entry : ossjAlarmUpdateList.entrySet()) {
                    LOG.debug("QosD sendAlarms() key:{}  AlarmValue.getAlarmChangedTime: {}",
                              entry.getKey().getPrimaryKey(), entry.getValue().getAlarmChangedTime());
                }
                LOG.debug("QosD sendAlarms() - ******* END OF LIST");
                LOG.debug("QosD sendAlarms() Sending alarm list to bean");
            }
            // send the alarmList to Ossbeans EJB or local runner via the
