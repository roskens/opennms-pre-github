...
/**
 * The Class ServiceBasedStrategyResolver.
 */
public class ServiceBasedStrategyResolver implements StrategyResolver {
     ...
     * Register.
     *
     * @return the service based strategy resolver
     */
    ...
    public SnmpStrategy getStrategy() {
        ...
        if (strategy == null) {
            if (m_strategies.isEmpty()) {
                throw new RuntimeException("There is no SnmpStrategy registered.  Unable to find strategy "
                        + strategyClass);
            } else {
