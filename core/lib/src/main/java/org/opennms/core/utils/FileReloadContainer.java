...
/**
 * <p>
 * Provides a container for returning an object and reloading the object if an
 * underlying file has changed. Ideally suited for automatically reloading
 ...
 * http://bugs.sun.com/bugdatabase/view_bug.do?bug_id=5096551 Leaving off the
 * <T> bits and using Object seem to work okay. --> There are two constructors:
 * <ol>
 * <li>{@link #FileReloadContainer(Object, Resource, FileReloadCallback)
 * FileReloadContainer(T, Resource, FileReloadCallback&lt;T&gt;)} is used for
 * objects having an underlying resource and are reloadable</li>
 * <li>{@link #FileReloadContainer(Object) FileReloadContainer(T)} is used for
 * objects that either do not have an underlying file or are otherwise not
 * reloadable</li>
 * </ol>
...
public class FileReloadContainer<T> {
    ...
    public FileReloadContainer(final T object, final Resource resource, final FileReloadCallback<T> callback) {
        ...
        try {
            ...
            LOG.info("Resource '{}' does not seem to have an underlying File object; assuming this is not an auto-reloadable file resource",
                     resource, e);
        }
    ...
    public synchronized void reload() {
        ...
        try {
            ...
            final String message = String.format("Failed reloading data for object '%s' from file '%s'. Unexpected Throwable received while issuing reload.",
                                                 m_object, m_file.getAbsolutePath());
            LOG.error(message, t);
            throw new DataAccessResourceFailureException(message, t);
        }

        if (object == null) {
            LOG.info("Not updating object for file '{}' due to reload callback returning null.",
     ...
     *         file will only be reloaded if {@link #reload()} is explicitly
     *         called.
     */
     ...
     *            indicates that automatic reload checks are not performed and
     *            the
     *            file will only be reloaded if {@link #reload()} is explicitly
     *            called.
     */
