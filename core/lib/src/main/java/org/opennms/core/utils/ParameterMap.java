 ...
 * map.
 *
 * @deprecated This class *modifies* the maps that are passed in, we should
 *             really do it another way.
 */
        ...
        if (oValue != null && oValue instanceof int[]) {
            ...
            while (ints.hasMoreElements()) {
                ...
                try {
                    ...
                    LOG.warn("getKeyedIntegerArray: failed to convert value {} to int array for key {} due to value {}",
                             oValue, key, token, e);
                }
            }
