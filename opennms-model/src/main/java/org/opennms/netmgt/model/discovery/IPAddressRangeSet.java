    ...
    public void add(IPAddressRange range) {
        ...
        for (ListIterator<IPAddressRange> it = m_ranges.listIterator(); it.hasNext();) {
            ...
            if (working.comesBefore(r) && !working.adjoins(r)) {
                // We've found the insertion point so just insert it and we are
                // finished
                // go back one
                it.previous();
                ...
                // now we remove the one we just merged since it is part of the
                // working range
                it.remove();
            } else {
    ...
    public void remove(IPAddressRange range) {
        for (ListIterator<IPAddressRange> it = m_ranges.listIterator(); it.hasNext();) {
            ...
            if (remaining.length == 0) {
                // 1. r is completely eclipse by range to there is nothing
                // remaining
                // so we just remove it
                it.remove();
            } else if (remaining.length == 1) {
                // 2. r overlaps the end of range so we have just the remaining
                // end
                ...
                // 4. range is right in the middle of r so we have both ends or
                // r
