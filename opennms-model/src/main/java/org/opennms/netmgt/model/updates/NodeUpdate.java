...
public class NodeUpdate {
	...
	public void setDeleteNode(boolean delete) {
		m_delete = delete;
	}
	...
	public Collection<IpInterfaceUpdate> getIpAddressUpdates() {
		return m_ipInterfaces.values();
	}
	
	public IpInterfaceUpdate getPrimaryIpInterfaceUpdate() {
		for (final IpInterfaceUpdate update : m_ipInterfaces.values()) {
			if (update.isSnmpPrimary()) {
				return update;
			}
		}
		return null;
	...
	public NodeUpdate addCategory(final String categoryName) {
		category(categoryName);
		return this;
	}

	public boolean hasCategory(final String categoryName) {
		return m_categoryUpdates.containsKey(categoryName);
	}

	public Collection<CategoryUpdate> getCategoryUpdates() {
		return m_categoryUpdates.values();
	}
	...
	public OnmsNode apply(final OnmsNode node, boolean recursive) {
		m_applied = true;
		...
		/* FIXME: Do we need to get a DAO to do this properly?
	...
	public String toString() {
		return new ToStringBuilder(this)
			...
			.append("labelSource", m_labelSource)
			.append("type", m_type)
			.append("netBiosName", m_netBiosName)
			.append("netBiosDomain", m_netBiosDomain)
			.append("operatingSystem", m_operatingSystem)
			.append("lastCapsdPoll", m_lastCapsdPoll)
			.append("ipInterfaces", m_ipInterfaces.values())
			.append("categories", m_categoryUpdates)
			.append("assetRecord", m_assetRecordUpdate)
			.append("sysObjectId", m_sysObjectId)
			.append("sysName", m_sysName)
			.append("sysDescription", m_sysDescription)
			.append("sysLocation", m_sysLocation)
			.toString();
	}
}
