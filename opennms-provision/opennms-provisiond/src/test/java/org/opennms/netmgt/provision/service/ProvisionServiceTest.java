...
public class ProvisionServiceTest implements InitializingBean {
     ...
     * - need a layer that takes the 'change set' and then persists it including
     * sending events
     ...
     * - some run once (ie per node) but update data many resources (eg
     * interfaces)
     * - some just look at interface attributes and update
     * - what do I do about scanners that want to lookup information from the
     * database...
     * - how do I deal with the transactions?
     * - I could have scan phases - some that collect and then others that
     * update
     * - it would be nice to be able to 'merge' all of the snmpdata that needs
     * to be
     ...
     * - a phase for collecting data and a phase for updating data (maybe pre
     * and post update?)
     * - maybe provide 'triggers' for each kind of resource added and be able to
     * associate
     * a scanner with a trigger
     * - as far as 'provided' data vs 'discovered' data we could have wrappers
     * that prevent
     ...
     * 1. agent scan phase
     * 2. phase for
     * I had the idea that we should have the scanners define new resources and
     * then
     * rework the collector to collect data for the resources that exist.
     * We can do scanners in multiple protocols SNMP, WMI, NRPE/NsClient,
     * others?
     * Two possible ideas related to this:
     * 1. We can make the scanners be responsible for finding not the
     * collectors.
     ...
     * - agentDetection -
     * - resourceDiscovery - scope Agent (calls Scope network, then node for
     * each node, then agent for matching agents)
     * - resourceScan - scope Resource (calls networ,
     * - persist - scope Node
     */

    /*
     * We need to define idea of a life cycle. A lifecycle would define a set of
     * phases
     * that it would go through. Each phase could be a single call or a pile of
     * calls executed
     ...
     * Lifecycle phases could contain embedded Lifecycles. For example the
     * Import lifecycle
     * would embed a Node Scanning Lifecycle for each node being inserted or
     * updated.
