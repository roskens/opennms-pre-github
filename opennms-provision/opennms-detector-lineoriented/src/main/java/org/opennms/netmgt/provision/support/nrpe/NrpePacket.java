...
public class NrpePacket {
     ...
     *         object.
     * @throws org.opennms.netmgt.provision.support.nrpe.NrpeException
     *             if any.
    ...
    public static NrpePacket receivePacket(final InputStream i, final int padding) throws NrpeException, IOException {
        ...
        if (k < PACKET_SIZE) {
            throw new NrpeException("Received packet is too small.  " + "Received " + k + ", expected at least "
                    + PACKET_SIZE);
        }

        p.m_version = (short) ((positive(packet[0]) << 8) + positive(packet[1]));
        p.m_type = (short) ((positive(packet[2]) << 8) + positive(packet[3]));
        long crc_l = ((long) positive(packet[4]) << 24) + ((long) positive(packet[5]) << 16)
                + ((long) positive(packet[6]) << 8) + ((long) positive(packet[7]));
     ...
     *            a int.
     * @param random
     *            a {@link java.security.SecureRandom} object.
     * @return an array of byte.
     */
