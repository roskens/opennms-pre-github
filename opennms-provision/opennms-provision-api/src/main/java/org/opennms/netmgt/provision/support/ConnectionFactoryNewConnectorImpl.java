 ...
 * asynchronous operations in MINA since it is creating new threads for each
 * socket operation.
 * </p>
 *
 * @author Seth
...
public class ConnectionFactoryNewConnectorImpl extends ConnectionFactory {
    ...
    private static final NioSocketConnector getSocketConnector(long timeout, IoHandler handler) {
        ...
        // Enable SO_REUSEADDR on the socket so that TIMED_WAIT connections that
        // are bound on the
        // same port do not block new outgoing connections. If the connections
        // are blocked, then
        ...
        // Setting SO_LINGER will prevent TIME_WAIT sockets altogether because
        // TCP connections will be
        // forcefully terminated with RST packets. However, this doesn't seem to
        // be necessary to maintain
        // performance of the outgoing connections. As long as SO_REUSEADDR is
        // set, the operating system
        // and Java appear to recycle the ports effectively although some will
    ...
    private static IoFutureListener<ConnectFuture> connectorDisposer(final SocketConnector connector) {
        return new IoFutureListener<ConnectFuture>() {
            ...
            public void operationComplete(ConnectFuture future) {
                try {
                    // Add a listener to the CloseFuture that will dispose of
