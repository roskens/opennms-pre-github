...
public class EventconfFactorySaveTest extends TestCase {
    ...
    public void testSave() throws Exception {
        ...
        m_eventConfDao.reload(); // The reload might happen as part of the
                                 // saveCurrent, but is not assured. We do so
                                 // here to be certain
        {
            List<Event> events = m_eventConfDao.getEvents(knownUEI1);
    ...
    public void testAddEventToProgrammaticStore() {
        ...
        // We are expecting this new file to be there - if it's not, that's an
        // issue
        m_fa.expecting(new File(m_fa.getTempDir().getAbsolutePath() + File.separator + "etc" + File.separator
