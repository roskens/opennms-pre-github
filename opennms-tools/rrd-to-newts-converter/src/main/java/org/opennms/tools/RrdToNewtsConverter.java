...
/**
 * The Class Rrd4J to Newts Converter
 * <p>
...
public class RrdToNewtsConverter {
     ...
     */
    private static final int m_statusThreadSleep = 5000;
    ...
    public NewtsRrdStrategy m_rrdStrategy;
    ...
    public RrdToNewtsConverter(String path, int threadCount, NewtsRrdStrategy rrdStrategy) {
        ...
        m_rrdStrategy.setConfigurationProperties(System.getProperties());
        BasicThreadFactory factory = new BasicThreadFactory.Builder()
                .namingPattern("rrd-newts-worker-%d")
                .daemon(true)
                .priority(Thread.NORM_PRIORITY)
                .build();
        m_executor = Executors.newFixedThreadPool(threadCount, factory);
    }
    ...
    private static void usage(final Options options, final CommandLine cmd, final String error, final Exception e) {
        ...
        try (PrintWriter pw = new PrintWriter(System.out)) {
            ...
            formatter.printHelp("Usage: Rrd4JToNewtsConverter <path>", options);
        ...
        for (File file : fileSet) {
            if (file.isDirectory()) {
                ...
                if (file.getName().endsWith(FILE_TYPE.JRB.ext())) {
                    ...
                    if (fileSet.contains(new File(filename + FILE_TYPE.XML.ext())) && fileSet.contains(new File(filename + FILE_TYPE.NEWTS.ext()))) {
                        m_skippedFiles++;
                    } else {
