...
public class ResourceTypeUtilsTest {
    ...
    public void testLoadPropertiesNullRelativePath() {
        ...
        try {
            //ResourceTypeUtils.loadProperties(null);
            ResourceTypeUtils.getStringProperties(Paths.get(""), (String) null);
        } catch (Throwable t) {
            ta.throwableReceived(t);
        }
        ta.verifyAnticipated();
    ...
    public void testLoadPropertiesEmpty() throws Exception {
        ...
        Properties p = ResourceTypeUtils.getStringProperties(m_pathAnticipator.getTempDir(), Paths.get("snmp", "1", "eth0"));
    ...
    public void testGetAttributesAtRelativePathWithBogusDirectory() {
        Path bogusRrdDirectory = Paths.get("/foo/bogus/blam/cheese/this/really/should/never/exist");
        assertFalse("bogus RRD directory " + bogusRrdDirectory + " should not exist", Files.exists(bogusRrdDirectory));
        ResourceTypeUtils.getAttributesAtRelativePath(bogusRrdDirectory, Paths.get("also-should-never-exist"));
    }
    ...
    public void testUpdateFileOutsideResourceTypeUtils() throws Exception {
        ...
        Path propertiesFile = resourceDir.resolve("strings.properties");
        ...
        try (Writer writer = Files.newBufferedWriter(propertiesFile, Charset.defaultCharset());) {
            properties.store(writer, "Updated!");
        }
    ...
    private OnmsResource createResource() {
        ...
        attributes.add(new RrdGraphAttribute("foo", Paths.get("1", "eth0"), "foo.jrb"));
    ...
    private Path createPropertiesFile(OnmsResource childResource, String propertiesContent, boolean onlyCreateParentDirectories) throws IOException {
        m_pathAnticipator.initialize();
        m_snmp = m_pathAnticipator.tempDir(ResourceTypeUtils.SNMP_DIRECTORY);
        m_node = m_pathAnticipator.tempDir(m_snmp, childResource.getParent().getName());
        m_intf = m_pathAnticipator.tempDir(m_node, childResource.getName());
        if (onlyCreateParentDirectories) {
            return m_intf.resolve("strings.properties");
        } else {
            return m_pathAnticipator.tempFile(m_intf, "strings.properties", propertiesContent);
        }
    }
