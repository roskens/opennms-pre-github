...
public class GenericIndexResourceTypeTest extends TestCase {
    ...
    public void testGetResourceByNodeAndIndexGetLabelPlain() {
        ...
        expect(m_resourceDao.getRrdDirectory()).andReturn(Paths.get("/a/bogus/directory"));
    ...
    public void testGetResourceByNodeAndIndexGetLabelStringAttribute() throws Exception {
        ...
        expect(m_resourceDao.getRrdDirectory()).andReturn(m_pathAnticipator.getTempDir());

        Path snmpDir = m_pathAnticipator.tempDir("snmp");
        Path snmpNodeDir = m_pathAnticipator.tempDir(snmpDir, "1");
        Path fooDir = m_pathAnticipator.tempDir(snmpNodeDir, "foo");
        Path indexDir = m_pathAnticipator.tempDir(fooDir, "1");
        m_pathAnticipator.tempFile(indexDir, ResourceTypeUtils.STRINGS_PROPERTIES_FILE_NAME, "stringAttribute=hello!!!!");
    ...
    public void testGetResourceByNodeSourceAndIndexGetLabelStringAttribute() throws Exception {
        ...
        Path snmpDir = m_pathAnticipator.tempDir("snmp");
        Path forSrcDir = m_pathAnticipator.tempDir(snmpDir, "fs");
        Path snmpNodeSourceDir = m_pathAnticipator.tempDir(forSrcDir, "source1");
        Path forIdDir = m_pathAnticipator.tempDir(snmpNodeSourceDir, "123");
        Path fooDir = m_pathAnticipator.tempDir(forIdDir, "foo");
        Path indexDir = m_pathAnticipator.tempDir(fooDir, "1");
        m_pathAnticipator.tempFile(indexDir, ResourceTypeUtils.STRINGS_PROPERTIES_FILE_NAME, "stringAttribute=hello!!!!");
