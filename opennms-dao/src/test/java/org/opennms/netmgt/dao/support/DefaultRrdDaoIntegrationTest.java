...
public class DefaultRrdDaoIntegrationTest extends TestCase {
    ...
    public void setUp() throws Exception {
        ...
        m_dao.setRrdBaseDirectory(m_pathAnticipator.getTempDir());
    ...
    public void testPrintValue() throws Exception {
        ...
        OnmsAttribute attribute = new RrdGraphAttribute("ifInOctets", Paths.get("snmp", "1", "eth0"), "ifInOctets.jrb");
        ...
        Path snmp = m_pathAnticipator.tempDir(ResourceTypeUtils.SNMP_DIRECTORY);
        Path node = m_pathAnticipator.tempDir(snmp, topResource.getName());
        Path intf = m_pathAnticipator.tempDir(node, childResource.getName());

        RrdDataSource rrdDataSource = new RrdDataSource(attribute.getName(), "GAUGE", 600, "U", "U");
        Object def = m_rrdStrategy.createDefinition("test", intf, attribute.getName(), 600, Collections.singletonList(rrdDataSource), Collections.singletonList("RRA:AVERAGE:0.5:1:100"));
        m_rrdStrategy.createFile(def, null);
        Path rrdFile = m_pathAnticipator.expecting(intf, attribute.getName() + RrdUtils.getExtension());

        Object rrdFileObject = m_rrdStrategy.openFile(rrdFile);
        for (int i = 0; i < 10; i++) {
    ...
    public void testNMS4861() throws Exception
    {
        ...
        final Path workDir = Paths.get("src", "test", "resources");
    	final RrdGraphDetails details = m_rrdStrategy.createGraphReturnDetails(command, workDir);
        final Path outputFile = Files.createTempFile("img", "png");
        IOUtils.copy(details.getInputStream(), Files.newOutputStream(outputFile));
