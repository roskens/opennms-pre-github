...
public class DefaultResourceDaoTest {
    ...
    private void setUpCollectdConfigFactory() throws MarshalException, ValidationException, IOException {
        try (InputStream stream = ConfigurationTestUtils.getInputStreamForResource(this, "/collectdconfiguration-testdata.xml")) {
    ...
    public void testGetResourceByIdNewTopLevelOnly() throws Exception {
        ...
        Path responseDir = m_pathAnticipator.tempDir("snmp");
        Path nodeDir = m_pathAnticipator.tempDir(responseDir, node.getId().toString());
        m_pathAnticipator.tempFile(nodeDir, "foo" + RrdUtils.getExtension());
    ...
    public void testGetResourceByIdNewTwoLevel() throws Exception {
        ...
        Collection<LocationMonitorIpInterface> locMons = new HashSet<>();
        ...
        Path response = m_pathAnticipator.tempDir("response");
        Path ipDir = m_pathAnticipator.tempDir(response, "192.168.1.1");
    ...
    public void testGetTopLevelResourceNodeSourceExists() throws Exception {
        ...
        Path responseDir = m_pathAnticipator.tempDir("snmp");
        Path forSrcDir = m_pathAnticipator.tempDir(responseDir, "fs");
        Path sourceDir = m_pathAnticipator.tempDir(forSrcDir, "source1");
        Path idDir = m_pathAnticipator.tempDir(sourceDir, "123");
        m_pathAnticipator.tempFile(idDir, "foo" + RrdUtils.getExtension());
    ...
    public void testGetTopLevelResourceDomainExists() throws IOException {
        Path snmp = m_pathAnticipator.tempDir("snmp");
        Path domain = m_pathAnticipator.tempDir(snmp, "example1");
        Path intf = m_pathAnticipator.tempDir(domain, "server1");
        m_pathAnticipator.tempFile(intf, "ifInOctects" + RrdUtils.getExtension());
    ...
    public void testGetTopLevelResourceDomainDoesNotExistInCollectdConfig() {
        ThrowableAnticipator ta = new ThrowableAnticipator();
        ta.anticipate(new ObjectRetrievalFailureException(OnmsResource.class, "bogus", "Domain not found due to domain RRD directory not existing or not a directory: " + m_pathAnticipator.getTempDir().resolve("snmp").resolve("bogus"), null));
    ...
    public void testGetTopLevelResourceDomainDoesNotExistNoInterfaceDirectories() throws IOException {
        Path snmp = m_pathAnticipator.tempDir("snmp");
        m_pathAnticipator.tempDir(snmp, "example1");

        ThrowableAnticipator ta = new ThrowableAnticipator();
        Path dir = m_pathAnticipator.getTempDir().resolve("snmp").resolve("example1");
        ta.anticipate(new ObjectRetrievalFailureException(OnmsResource.class, "example1", "Domain not found due to domain RRD directory not matching the domain directory filter: " + dir.toAbsolutePath(), null));
    ...
    public void testFindNodeResourcesWithDistributedResponseTime() throws Exception {
        ...
        Path response = m_pathAnticipator.tempDir("response");
        Path distributed = m_pathAnticipator.tempDir(response, "distributed");
        Path monitor = m_pathAnticipator.tempDir(distributed, "1");
        Path ipDir = m_pathAnticipator.tempDir(monitor, "192.168.1.1");
        m_pathAnticipator.tempFile(ipDir, "icmp" + RrdUtils.getExtension());
    ...
    public void testFindNodeResourcesWithNodeSnmp() throws Exception {
        ...
        Path snmp = m_pathAnticipator.tempDir("snmp");
        Path nodeDir = m_pathAnticipator.tempDir(snmp, "1");
        m_pathAnticipator.tempFile(nodeDir, "foo" + RrdUtils.getExtension());
    ...
    public void testFindNodeResourcesWithNodeInterface() throws Exception {
        ...
        Path snmp = m_pathAnticipator.tempDir("snmp");
        Path nodeDir = m_pathAnticipator.tempDir(snmp, "1");
        Path intfDir = m_pathAnticipator.tempDir(nodeDir, "eth0");
        m_pathAnticipator.tempFile(intfDir, "foo" + RrdUtils.getExtension());
...
//        expect(m_locationMonitorDao.findStatusChangesForNodeForUniqueMonitorAndInterface(node.getId())).andReturn(new ArrayList<LocationMonitorIpInterface>(0));
    ...
    public void testGetResourceForIpInterfaceWithLocationMonitor() throws Exception {
        ...
        Path response = m_pathAnticipator.tempDir("response");
        Path distributed = m_pathAnticipator.tempDir(response, "distributed");
        Path locMonDir = m_pathAnticipator.tempDir(distributed, locMon.getId().toString());
        Path ipDir = m_pathAnticipator.tempDir(locMonDir, InetAddressUtils.str(ip.getIpAddress()));
        m_pathAnticipator.tempFile(ipDir, "http" + RrdUtils.getExtension());
...
//        expect(m_locationMonitorDao.findStatusChangesForNodeForUniqueMonitorAndInterface(node.getId())).andReturn(new ArrayList<LocationMonitorIpInterface>(0));
    ...
    public void testFindNodeSourceDirectoriesNoRrdFiles() throws Exception {
        ...
        Path sourceDir = m_pathAnticipator.tempDir(forSrcDir, "source1");
        Path idDir = m_pathAnticipator.tempDir(sourceDir, "123");
        m_pathAnticipator.tempFile(idDir, "foo");
