...
public class LocationMonitorDaoHibernate extends AbstractDaoHibernate<OnmsLocationMonitor, Integer> implements LocationMonitorDao {
     ...
     * {@inheritDoc}
     *
     * @deprecated  Don't call this for now.
     */
    @Override
    @Deprecated
    ...
    public OnmsLocationSpecificStatus getMostRecentStatusChange(final OnmsLocationMonitor locationMonitor, final OnmsMonitoredService monSvc) {
    	return getJpaTemplate().execute(new JpaCallback<OnmsLocationSpecificStatus>() {

            @Override
            public OnmsLocationSpecificStatus doInJpa(final EntityManager em) {
                return (OnmsLocationSpecificStatus)em.createQuery("from OnmsLocationSpecificStatus status where status.locationMonitor = :locationMonitor and status.monitoredService = :monitoredService order by status.pollResult.timestamp desc")
                    .setParameter("locationMonitor", locationMonitor)
                    .setParameter("monitoredService", monSvc)
                    .setMaxResults(1)
                    .getSingleResult();
            }
        });
    ...
    public Collection<OnmsLocationSpecificStatus> getStatusChangesBetweenForApplications(final Date startDate, final Date endDate, final Collection<String> applicationNames) {
        return getJpaTemplate().execute(new JpaCallback<Collection<OnmsLocationSpecificStatus>>() {

            @SuppressWarnings("unchecked")
            @Override
            public List<OnmsLocationSpecificStatus> doInJpa(EntityManager em) {
                
                return (List<OnmsLocationSpecificStatus>)em.createQuery(
                ...
                .setParameter("applicationNames", applicationNames)
    ...
    public Collection<LocationMonitorIpInterface> findStatusChangesForNodeForUniqueMonitorAndInterface(final int nodeId) {
		...
		@SuppressWarnings("unchecked")
        final List<Object[]> l = getJpaTemplate().find(
        ...
        getJpaTemplate().execute(
            new JpaCallback<Void>() {
                ...
                public Void doInJpa(EntityManager em) throws PersistenceException {
                    em.createQuery("update OnmsLocationMonitor as mon set mon.status = ? where mon.status != ?")
                        .setParameter("whereStatus", MonitorStatus.STOPPED)
                        .setParameter("setStatus", MonitorStatus.PAUSED)
                        .executeUpdate();
                    return null;
                }
            });
    ...
    public void resumeAll() {
        getJpaTemplate().execute(
            new JpaCallback<Void>() {
                ...
                public Void doInJpa(EntityManager em) throws PersistenceException {
                    em.createQuery("update OnmsLocationMonitor as mon set mon.status = ? where mon.status != ?")
                            .setParameter("whereStatus", MonitorStatus.PAUSED)
                            .setParameter("setStatus", MonitorStatus.STARTED)
                            .executeUpdate();
                    return null;
                }
            });
