...
/**
 * A UpsertTemplate for handling the update if it exists/insert if it doesn't
 * case in the midst of
 * concurrent database updates. The pattern for solving this is known as an
 * Upsert.. update or insert.
 * Example Scenario: During a node scan an interface is found on a node and
 * various information about
 * this interface is gathered. This information needs to be persisted to the
 ...
 * The naive implementation of this is something like the following (this is
 * just pseudo code)
 ...
 * }
 * Problem: The naive implementation above has the problem that it fails in the
 * midst of concurrency.
 * Consider the following scenario where two different provisioning threads
 ...
 * 5 Thread 2 attempts to insert the if into the database and a duplication
 * exception is thrown
 * 6 All work done in Thread 2's transactions is rolled back.
 * Most people assume the 'transactions' will handle this case but they do not.
 * The reason for this is
 * because transactions lock the information that is found to ensure that this
 * information is not changed
 * by others. However when you perform a query that returns nothing there is
 * nothing to lock. So this case
 * is not protected by the transaction.
 * Solution: To handle this we must execute the insert in a 'sub transaction'
 * and retry in the event of failure: The
 ...
 * }
 * }
 * This is simplified loop because it does not show all of the code to start
 * transactions and such nor to it show
 * the real.
 * As far as code goes this solution has a great deal of boiler plate code. This
 * class contains this boilerplate
 * code using the Template Method pattern and provides abstract methods for
 * filling running the query and for doing the
 * insert and/or the update. To use this class to do the above would look
 * something like this:
 ...
 *           }
 *           }.execute();
 *           The above will handle all of the exceptions that can occur in the
 *           face of concurrent inserts and will properly either
...
public abstract class UpsertTemplate<T, D extends OnmsDao<T, ?>> {
    ...
    private T doUpsert() {
        ...
        // lock the table since we are about to insert and don't want it
        // inserted
        m_dao.lock();
    ...
    /**
     * Called by doUpsert to insert the object. This method starts a new
     * transaction
    ...
    /**
     * Override this method to insert a new object into the database. This
     * method will be called
     * when the query method returns null. A DataIntegrityViolationException
     * should be thrown if
