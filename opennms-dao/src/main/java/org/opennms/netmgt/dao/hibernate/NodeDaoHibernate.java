...
public class NodeDaoHibernate extends AbstractDaoHibernate<OnmsNode, Integer> implements NodeDao {
    ...
    public OnmsNode getHierarchy(Integer id) {
        ...
        // TODO MVR JPA refactor me (hint: could be a fetch)
...
//        return node;
    ...
    public List<OnmsNode> findAllByCategoryLists( final Collection<OnmsCategory> rowCategories, final Collection<OnmsCategory> columnCategories) {
        ...
        return getJpaTemplate().execute(new JpaCallback<List<OnmsNode>>() {

            @Override
            public List<OnmsNode> doInJpa(EntityManager em)  {
                return (List<OnmsNode>) em.createQuery("select distinct n from OnmsNode as n "
                        ...
                        + "and n.type != 'D'")
                        .setParameter("rowCategories", rowCategories)
                        .setParameter("colCategories", columnCategories)
                        .getResultList();
            }
        });
    ...
    public SurveillanceStatus findSurveillanceStatusByCategoryLists(final Collection<OnmsCategory> rowCategories, final Collection<OnmsCategory> columnCategories) {
        return getJpaTemplate().execute(new JpaCallback<SurveillanceStatus>() {

            @Override
            public SurveillanceStatus doInJpa(EntityManager em) {

                throw new RuntimeException("not yet refactored");
                //TODO MVR JPA
//                return (SimpleSurveillanceStatus)em.createQuery("select" +
//                        " new " + SimpleSurveillanceStatus.class.getName() + "(" +
...
//                		)
//                		.setParameter("rowCategories", rowCategories)
//                		.setParameter("columnCategories", columnCategories)
//                        .getSingleResult();
            }
        });
    }
    ...
    public Map<String, Integer> getForeignIdToNodeIdMap(String foreignSource) {
        List<Object[]> pairs = getJpaTemplate().find("select n.id, n.foreignId from OnmsNode n where n.foreignSource = ?", foreignSource);
    ...
    public void deleteObsoleteInterfaces(final Integer nodeId, final Date scanStamp) {
        getJpaTemplate().execute(new JpaCallback<Void>() {
            ...
            public Void doInJpa(EntityManager em) throws PersistenceException {
                em.createQuery("delete from OnmsSnmpInterface snmpInterface where snmpInterface.node.id = :nodeId and (snmpInterface.lastCapsdPoll is null or snmpInterface.lastCapsdPoll < :scanStamp)")
                    .setParameter("nodeId", nodeId)
                    .setParameter("scanStamp", scanStamp)
                    .executeUpdate();

                em.createQuery("delete from OnmsIpInterface ipInterface where ipInterface.node.id = ? and ipInterface.isSnmpPrimary != 'P' and (ipInterface.ipLastCapsdPoll is null or ipInterface.ipLastCapsdPoll < ?)")
                        .setParameter("nodeId", nodeId)
                        .setParameter("scanStamp", scanStamp)
                        .executeUpdate();
                return null;
