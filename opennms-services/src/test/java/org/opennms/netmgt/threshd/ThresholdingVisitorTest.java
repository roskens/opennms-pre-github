...
public class ThresholdingVisitorTest {
    ...
    PathAnticipator m_pathAnticipator;
    Map<Integer, Path> m_hrStorageProperties;
    ...
    public void setUp() throws Exception {
        ...
        m_hrStorageProperties = new HashMap<Integer, Path>();
        ...
        sb.append("</outage>");
        sb.append("</outages>");
        Path file = Paths.get("target", "poll-outages.xml");
        try (Writer writer = Files.newBufferedWriter(file, Charset.defaultCharset());) {
            writer.write(sb.toString());
        }
        PollOutagesConfigFactory.setInstance(new PollOutagesConfigFactory(new FileSystemResource(file.toFile())));
...
//        MockLogAppender.assertNoWarningsOrGreater();
    ...
    public void tearDown() throws Exception {
        ...
        m_pathAnticipator.deleteExpected(true);
    ...
    public void testInterfaceResourceWithStringAttributeFilter() throws Exception {
        ...
        Path nodeDir = m_pathAnticipator.tempDir("1");
        Path resourceDir = m_pathAnticipator.tempDir(nodeDir, ifName);
        Path propertiesFile = m_pathAnticipator.tempFile(resourceDir, "strings.properties");
    ...
    public void testThresholdsFiltersOnNodeResource() throws Exception {
        ...
        Path nodeDir = m_pathAnticipator.tempDir("1");
        Path propertiesFile = m_pathAnticipator.tempFile(nodeDir, "strings.properties");
        ResourceTypeUtils.saveUpdatedProperties(propertiesFile, p);
    ...
    private Path createHrStoragePropertiesFile(int resourceId) throws IOException {
        if (!m_hrStorageProperties.containsKey(resourceId)) {
            Path nodeDir = m_pathAnticipator.getTempDir().resolve("1");
            if (!Files.exists(nodeDir)) {
                nodeDir = m_pathAnticipator.tempDir("1");
            }
            Path hrStorageDir = nodeDir.resolve("hrStorageIndex");
            if (!Files.exists(hrStorageDir)) {
                hrStorageDir = m_pathAnticipator.tempDir(nodeDir, "hrStorageIndex");
            }
            Path resourceDir = m_pathAnticipator.tempDir(hrStorageDir, String.valueOf(resourceId));
            m_hrStorageProperties.put(resourceId, m_pathAnticipator.tempFile(resourceDir, "strings.properties"));
        }
    ...
    private static SnmpCollectionAgent createCollectionAgent() {
        ...
        EasyMock.expect(agent.getStorageDir()).andReturn(Paths.get(String.valueOf(1))).anyTimes();
    ...
    private RrdRepository getRepository() {
        ...
        repo.setRrdBaseDir(m_pathAnticipator.getTempDir());
        return repo;
    }
