...
/**
 * Implementation of the <code>Tl1MessageProcessor</code> Interface. This is the
 * default
 * Autonomous Message Parser based on Tl1Messages recorded from the Hitachi
 * GPOND TL1 simulator.
 *
...
public class Tl1AutonomousMessageProcessor implements Tl1MessageProcessor {
     ...
     * (non-Javadoc)
     * @see
     * org.opennms.netmgt.tl1d.Tl1MessageProcessor#process(java.lang.String,
     * int)
     */
    /** {@inheritDoc} */
    @Override
    ...
    private static boolean parseHeader(String line, Tl1AutonomousMessage message) throws IllegalArgumentException {
        ...
        try {
            if (message.getHeader().getDate().matches("^[0-9]{4}")) {
                message.getHeader().setTimestamp(SDF_4DY.get().parse(message.getHeader().getDate() + " "
                                                                             + message.getHeader().getTime()));
            } else {
                message.getHeader().setTimestamp(SDF_2DY.get().parse(message.getHeader().getDate() + " "
                                                                             + message.getHeader().getTime()));
            }
            message.setTimestamp(message.getHeader().getTimestamp());
        } catch (ParseException e) {
            throw new IllegalArgumentException("The line: " + line + ", doesn't contain date and time in the format: "
                    + SDF_2DY.get().toLocalizedPattern() + " or " + SDF_4DY.get().toLocalizedPattern());
        }
    ...
    private boolean parseId(String line, Tl1AutonomousMessage message) {
        ...
        if (idParser.countTokens() < 3) {
            throw new IllegalArgumentException("The line: " + line
                    + " is not an Autonomouse message id.  Expected 3 or more tokens " + "and received: "
                    + idParser.countTokens());
        }
    ...
    private boolean parseAutoBlock(String line, Tl1AutonomousMessage message) {
        ...
        // Also we could have muliple messages in this block. Need to handle
        // later.
