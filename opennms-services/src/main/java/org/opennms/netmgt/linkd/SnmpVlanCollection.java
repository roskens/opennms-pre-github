...
public final class SnmpVlanCollection implements ReadyRunnable {
    ...
    public void run() {
        ...
        if (m_collectBridge && m_collectStp) {
            walker = SnmpUtils.createWalker(m_agentConfig,
                                            "dot1dBase/dot1dBaseTable/dot1dStp/dot1dStpTable/dot1dTpFdbTable ",
                                            new CollectionTracker[] { m_dot1dBase, m_dot1dBaseTable, m_dot1dStp,
                                                    m_dot1dStpTable, m_dot1dTpFdbTable });
        } else if (m_collectBridge) {
            walker = SnmpUtils.createWalker(m_agentConfig, "dot1dTpFdbTable ",
                                            new CollectionTracker[] { m_dot1dTpFdbTable });
        } else if (m_collectStp) {
            walker = SnmpUtils.createWalker(m_agentConfig, "dot1dBase/dot1dStp/dot1dBaseTable/dot1dStpTable ",
                                            new CollectionTracker[] { m_dot1dBase, m_dot1dStp, m_dot1dBaseTable,
                                                    m_dot1dStpTable });
        } else {
        ...
        if (m_dot1dTpFdbTable.isEmpty() && m_collectBridge) {
            LOG.info("run: Trying to collect QbridgeDot1dTpFdbTable for {} Community: {}", hostAddress,
                     m_agentConfig.getReadCommunity());
            m_dot1qTpFdbTable = new QBridgeDot1dTpFdbTable(m_address);
            walker = SnmpUtils.createWalker(m_agentConfig, "qBridgedot1dTpFdbTable ",
            ...
            if (!hasQBridgeDot1dTpFdbTable()) {
                LOG.info("run: failed to collect QBridgeDot1dTpFdbTable for {} Community: {}", hostAddress,
                         m_agentConfig.getReadCommunity());
            }
        }
    }
    ...
    public String toString() {
        return new ToStringBuilder(this).append("agentConfig", m_agentConfig).append("address", m_address).append("dot1dBase",
                                                                                                                  m_dot1dBase).append("dot1dBaseTable",
                                                                                                                                      m_dot1dBaseTable).append("dot1dStp",
                                                                                                                                                               m_dot1dStp).append("dot1dStpTable",
                                                                                                                                                                                  m_dot1dStpTable).append("dot1dTpFdbTable",
                                                                                                                                                                                                          m_dot1dTpFdbTable).append("dot1qTpFdbTable",
