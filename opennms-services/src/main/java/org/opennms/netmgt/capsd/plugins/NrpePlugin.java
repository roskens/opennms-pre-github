...
public final class NrpePlugin extends AbstractPlugin {
     ...
     */
    private boolean isServer(InetAddress host, int port, String command, int padding, int retries, int timeout,
            RE regex, StringBuffer bannerResult) {
            ...
            try {
                ...
                if (response.getResultCode() == 0) {
                    ...
                    if (r.match(response_msg)) {
                        ...
                        LOG.info("received 1-2 return code, {}, with message: {}", response.getResultCode(),
                                 response.getBuffer());
                        isAServer = false;
                        break;
                    }
                } else {
                    LOG.info("received 3+ return code, {}, with message: {}", response.getResultCode(),
                             response.getBuffer());
                    isAServer = false;
                    break;
                }
                ...
                /*
                 * // If banner matching string is null or wildcard ("*") then
                 * we
                ...
                LOG.info("NrpePlugin: An expected I/O exception occured connecting to host {} on port {}",
                         InetAddressUtils.str(host), port, e);
                isAServer = false;
            } catch (Throwable t) {
                isAServer = false;
                LOG.warn("NrpePlugin: An undeclared throwable exception was caught connecting to host {} on port {}",
