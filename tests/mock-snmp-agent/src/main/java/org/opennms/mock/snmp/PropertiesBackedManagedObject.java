...
public class PropertiesBackedManagedObject implements ManagedObject, MockSnmpMOLoader, Updatable, MOAccess {
    ...
    public List<ManagedObject> loadMOs(URL moFile) {
        ...
        for (final Entry<Object, Object> e : props.entrySet()) {
            ...
            if (!key.startsWith(".")) {
                s_log.debug(String.format("key does not start with '.', probably a linewrap issue in snmpwalk: %s = %s",
                                          key, value));
                continue;
            }
            try {
                ...
                // Catch any malformed OIDs and create a more descriptive error
                // message
                final IllegalArgumentException nfe = new IllegalArgumentException("Could not load OID value: [" + key
                        + "] [" + value + "]");
                nfe.initCause(ex);
                throw nfe;
            }
        ...
        if ("\"\"".equals(valStr)) {
            ...
            try {
                ...
                if (moTypeStr.equals("STRING")) {
                    ...
                    newVar = new Counter32(Long.parseLong(moValStr)); // a 32
                                                                      // bit
                                                                      // counter
                                                                      // can be
                                                                      // > 2 ^
                    ...
                    Integer ticksInt = Integer.parseInt(moValStr.substring(moValStr.indexOf('(') + 1,
                                                                           moValStr.indexOf(')')));
    ...
    protected Variable handleDynamicVariable(String oidStr, String typeStr) {
        ...
        try {
            ...
            // If the singleton initialization failed, then create a new
            // instance
            if (responder == null) {
