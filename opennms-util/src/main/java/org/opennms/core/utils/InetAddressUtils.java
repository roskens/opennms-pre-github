...
public abstract class InetAddressUtils {
    ...
    static {
        ...
        try {
            // This address is within a subnet of "Unique Unicast" IPv6
            // addresses
            // that are defined by RFC4193. This is the IPv6 equivalent of the
            // 192.168.0.0/16 subnet and because the IPv6 address space is so
            // large,
            // you can just randomly generate the first portion of the address.
            // :)
            // I used an online address generator to get this particular
            // address.
            //
 ...
 IPv6
    ...
    public static InetAddress getLocalHostAddress() {
        try {
            ...
            LOG.warn("getLocalHostAddress: Could not lookup the host address for the local host machine, address set to '127.0.0.1'.",
                     e);
    ...
    public static InetAddress getInetAddress(final byte[] ipAddrOctets) {
        try {
            ...
            throw new IllegalArgumentException("Invalid IPAddress " + ipAddrOctets + " with length "
                    + ipAddrOctets.length);
        }
    ...
    public static InetAddress resolveHostname(final String hostname, final boolean preferInet6Address,
        ...
        // preferInet6Address ? "6" : "4"));

        // Do a special case for localhost since the DNS server will generally
        // not
        ...
        try {
            // 2011-05-22 - Matt is seeing some platform-specific
            // inconsistencies when using
            // InetAddress.getAllByName(). It seems to miss some addresses
            // occasionally on Mac.
            // We need to use dnsjava here instead since it should be 100%
            // reliable.
            //
            ...
            try {
                ...
                if (aRecs != null) {
                        ...
                        if (aRec instanceof ARecord) {
                            ...
                            if (addr instanceof Inet4Address) {
                                ...
                                throw new UnknownHostException(
                                                               "Non-IPv4 address found via A record DNS lookup of host: "
                    ...
                    // throw new
                    // UnknownHostException("No IPv4 addresses found via A record DNS lookup of host: "
                    // + hostname);
                }
            } catch (final TextParseException e) {
                final UnknownHostException ex = new UnknownHostException("Could not perform A record lookup for host: "
                        + hostname);
                ex.initCause(e);
                throw ex;
            }
                ...
                if (quadARecs != null) {
                    for (final Record quadARec : quadARecs) {
                        ...
                        if (addr instanceof Inet6Address) {
                            ...
                            // Should never happen
                            throw new UnknownHostException(
                                                           "Non-IPv6 address found via AAAA record DNS lookup of host: "
                                                                   + hostname + ": " + addr.toString());
                        }
                    }
                } else {
                    // throw new
                    // UnknownHostException("No IPv6 addresses found via AAAA record DNS lookup of host: "
                    // + hostname);
                }
            } catch (final TextParseException e) {
                final UnknownHostException ex = new UnknownHostException(
                                                                         "Could not perform AAAA record lookup for host: "
                                                                                 + hostname);
                ex.initCause(e);
                throw ex;
            }
            ...
            if (throwException) {
                ...
                // System.out.println(String.format("UnknownHostException for : %s (%s)",
                // hostname, preferInet6Address ? "6" : "4"));
                // e.printStackTrace();
                return null;
            }
        }
    ...
    public static String toIpAddrString(final InetAddress addr) {
        if (addr == null) {
            ...
            if (address == null) {
                // This case can occur when Jersey uses Spring bean classes
                // which use
                // CGLIB bytecode manipulation to generate InetAddress classes.
                // This will
                // occur during REST calls. {@see
                // org.opennms.web.rest.NodeRestServiceTest}
                //
    ...
    public static String toIpAddrString(final byte[] addr) {
        if (addr.length == 4) {
            ...
            return String.format("%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x", addr[0],
                                 addr[1], addr[2], addr[3], addr[4], addr[5], addr[6], addr[7], addr[8], addr[9],
                                 addr[10], addr[11], addr[12], addr[13], addr[14], addr[15]).intern();
        } else {
    ...
    public static InetAddress convertBigIntegerIntoInetAddress(final BigInteger i) throws UnknownHostException {
        if (i.compareTo(BigInteger.ZERO) < 0) {
            ...
            // Note: This function will return the two's complement byte array
            // so there will always
            // be a bit of value '0' (indicating positive sign) at the first
            ...
            if (bytes.length == 0) {
                ...
                // This case covers an IPv4 address with the most significant
                // bit of zero (the MSB
                ...
                // This case covers an IPv4 address (4 bytes + two's complement
                // sign bit of zero)
                final byte[] addressBytes = new byte[4];
                int k = 3;
                ...
                // This case covers an IPv6 address with the most significant
                // bit of zero (the MSB
                // will be used as the two's complement sign bit)
                ...
                // This case covers an IPv6 address (16 bytes + two's complement
                // sign bit of zero)
                final byte[] addressBytes = new byte[16];
                int k = 15;
                for (int j = bytes.length - 1; j >= 1; j--, k--) {
                ...
                throw new IllegalArgumentException("BigInteger is too large to convert into an IP address: "
                        + i.toString());
            }
        }
    }
    ...
    /**
     * This function is used to ensure that an IP address string is in
     * fully-qualified
    ...
    public static byte[] macAddressStringToBytes(String macAddress) {
        ...
        if (digits.length != 6) {
            ...
            if (macAddress.length() == 12) {
                digits = new String[] { macAddress.substring(0, 2), macAddress.substring(2, 4),
        ...
        for (int i = 0; i < 6; i++) {
            // Prefix the value with "0x" so that Integer.decode() knows which
            // base to use
    ...
    public static String macAddressBytesToString(byte[] macAddress) {
        ...
        return String.format(
                             ...
                             "%02x%02x%02x%02x%02x%02x", macAddress[0], macAddress[1], macAddress[2], macAddress[3],
                             macAddress[4],
                             macAddress[5]);
    }
