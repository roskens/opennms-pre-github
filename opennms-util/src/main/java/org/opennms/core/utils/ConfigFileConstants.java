...
public final class ConfigFileConstants {
     ...
     * qualified name of the file is needed then use {@link #getFile(int)
     * getFile()}to returned the {@link java.io.File File} object. The java File
     * object can be queried to get more detailed
     ...
     * </p>
     * <p>
     * The file is looked for in the <em>etc</em> directory of the OpenNMS home
     * location identified by the System property <em>opennms.home</em>. If the
     * file is not found in the <em>etc</em> directory then an attempt is made
     * to find it in the root OpenNMS directory. If it still cannot be found
     * then a {@link java.io.FileNotFoundException FileNotFoundException} is
    ...
    public static File getFile(int id) throws IOException {
        ...
        if (!frfile.exists()) {
            ...
            if (!frfileNoEtc.exists()) {
                throw new FileNotFoundException("The requested file '" + rfile + "' could not be found at '"
                        + frfile.getAbsolutePath() + "' or '" + frfileNoEtc.getAbsolutePath() + "'");
            }
        }
        ...
        if (!frfile.exists()) {
            ...
            if (!frfileNoEtc.exists()) {
                throw new FileNotFoundException(
                                                String.format("The requested file '%s' could not be found at '%s' or '%s'",
                                                              fname, frfile.getAbsolutePath(),
