...
<form action="authority.selection.page" method="post">
    <div class="lista">
        <div>
            <spring:message code="authority.categories.assign"/>:<br/><br/>
            <select multiple="multiple" name="included" id="included" class="selectmultiple">
                <c:forEach var="itemAuthority" items="${authorityItems}" varStatus="status">
                    <option value="${itemAuthority.id}">${itemAuthority.name}</option>
                </c:forEach>
            </select>
        </div>
    ...
    <div class="pulsanti">
        ...
        <input type="submit" onclick="javascript:setInputList($('included'),$('includedHidden'));" value="<spring:message code="authority.categories.assignable"/>"/>
    </div>
</form>
<br/>