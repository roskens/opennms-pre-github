...
<springf:form action="authority.edit.page" commandName="authority" method="post">
	<table>
		<tr>
		 ...
		 <c:choose>
        	<c:when test="${authority.id == 0}">
			<td><springf:input path="name" maxlength="50" size="35"/></td>
			<td><springf:errors path="name" cssClass="error-style"/></td>
			</c:when>
        <c:otherwise>
        	<td>${authority.name}</td>
			<input type="hidden" id="name" name="name" value="${authority.name}"/> 
        </c:otherwise>
        </c:choose>
		</tr>
		<tr>
			<td><spring:message code="authority.description"/>:</td>
			<td><springf:textarea path="description" cols="30"/></td>
