...
<form action="authority.list.page" method="post">
    ...
    <p class="delimitato">
        ...
        <c:forEach var="groupMember" items="${user.groups}" varStatus="status">
            <option>${groupMember.group.name}</option>
            <c:forEach var="authority" items="${groupMember.group.authorities}" varStatus="status">
                <option>-->${authority.name}</option>
                <c:forEach var="category" items="${authority.categories}" varStatus="status">
    ...
    <input type="button" onclick="location.href = 'user.detail.page?sid=${user.username}'" value="<spring:message code="ui.action.edit"/>"/></p>
