    ...
    function submitForm(anAction)
    {
      ...
      %> 
	    ...
	    <% if(alarms[i].getId() > 0 ) { %>           
          ...
          <td class="divider">
            <nobr><span title="Event <%= alarms[i].getLastEventID() %>"><a href="event/detail.htm?id=<%= alarms[i].getLastEventID() %>"><element:formatDate date="${alarm.lastEventTime}"/></a></span></nobr>
            <nobr>
          ...
          <c:if test="${param.display == 'long'}">
          <br />
            <nobr><element:formatDate date="${alarm.firstEventTime}"/></nobr>
            <nobr>
