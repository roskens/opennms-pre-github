...
public class InventoryService implements InitializingBean {
    ...
    public Map<String, Object> getRancidNodeBase(int nodeid) {
        ...
        LOG.debug("getRancidNodeBase opennms.rancidIntegrationUseOnlyRancidAdapter: {}",
                  rancidIntegrationUseOnlyRancidAdapterProperty);
        if (rancidIntegrationUseOnlyRancidAdapterProperty != null
                && "true".equalsIgnoreCase(rancidIntegrationUseOnlyRancidAdapterProperty.trim())) {
    ...
    /*
     * getRancidNode will filter any exception, the page will show an empty
     * table
        ...
        while (iter1.hasNext()) {
            ...
            try {
                ...
                try {
                    ...
                    RancidNodeWrapper rnw = new RancidNodeWrapper(rn.getDeviceName(), groupname, rn.getDeviceType(),
                                                                  rn.getComment(), rn.getHeadRevision(),
                                                                  rn.getTotalRevisions(), in.getCreationDate(),
                                                                  rn.getRootConfigurationUrl());

                    ranlist.add(rnw);
                } catch (RancidApiException e) {
                    LOG.debug("No configuration found for nodeid:{} on Group: {} .Cause: {}", nodeid, groupname,
                              e.getLocalizedMessage());
                }
            } catch (RancidApiException e) {
                if (e.getRancidCode() == 2) {
                    LOG.debug("No device found in router.db for nodeid:{} on Group: {} .Cause: {}", nodeid, groupname,
                              e.getLocalizedMessage());
                } else {
                    nodeModel.put("RWSStatus", e.getLocalizedMessage());
                    LOG.error(e.getLocalizedMessage());
        ...
        while (iter2.hasNext()) {
            ...
            try {
                ...
                while (iter1.hasNext()) {
                    vs = iter1.next();
                    InventoryNode in = (InventoryNode) rn.getNodeVersions().get(vs);
                    InventoryWrapper inwr = new InventoryWrapper(in.getVersionId(), in.getCreationDate(), groupname,
                                                                 in.getConfigurationUrl());
                    ranlist.add(inwr);
                }
            } catch (RancidApiException e) {
                if (e.getRancidCode() == 2) {
    ...
    public Map<String, Object> getRancidNodeList(int nodeid, String group) {
        ...
        while (iter1.hasNext()) {
            vs = iter1.next();
            InventoryNode in = (InventoryNode) rn.getNodeVersions().get(vs);
            InventoryWrapper inwr = new InventoryWrapper(in.getVersionId(), in.getCreationDate(), group,
                                                         in.getConfigurationUrl());
            ranlist.add(inwr);
        }
    ...
    /*
     * getRancidNodeWithClogin will filter any exception, the page will show an
     * empty table
     * in case of node not in DB or device name not in RWS
     */
    /**
    ...
    public boolean deleteNodeOnRouterDb(String groupName, String deviceName) {
        ...
        try {
            ...
            LOG.debug("deleteNodeOnRouterDb has given exception on node {}/{} {}", groupName, deviceName,
                      e.getMessage());
            return false;
        }
        return true;
     ...
     * @return a boolean.
     */
    public boolean updateNodeOnRouterDb(String groupName, String deviceName, String deviceType, String status,
            String comment) {

        LOG.debug("InventoryService updateNodeOnRouterDb: {}->{}:{}:{}:{}", groupName, deviceName, deviceType, status,
                  comment);
        ...
        try {
            ...
            LOG.debug("updateNodeOnRouterDb has given exception on node {}/{} {}", groupName, deviceName,
                      e.getMessage());
            return false;
        }
        return true;
     ...
     * @return a boolean.
     */
    public boolean createNodeOnRouterDb(String groupName, String deviceName, String deviceType, String status,
            String comment) {

        LOG.debug("InventoryService createNodeOnRouterDb: {}->{}:{}:{}:{}", groupName, deviceName, deviceType, status,
                  comment);
        ...
        try {
            ...
            LOG.debug("createNodeOnRouterDb has given exception on node {}/{} {}", groupName, deviceName,
                      e.getMessage());
            return false;
        }
        return true;
     ...
     * @return a boolean.
     */
    public boolean updateClogin(String deviceName, String groupName, String userID, String pass, String enPass,
            String loginM, String autoE) {
        LOG.debug("InventoryService updateClogin for following changes userID [{}] pass [{}] enpass [{}] loginM [{}] autoE [{}] groupName (ignored) [{}] deviceName [{}]",
                  userID, pass, enPass, loginM, autoE, groupName, deviceName);
        try {
