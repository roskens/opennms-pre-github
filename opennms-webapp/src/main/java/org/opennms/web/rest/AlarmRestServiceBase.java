...
public class AlarmRestServiceBase extends OnmsRestService {
    ...
    static {
        ...
        m_severityPattern = Pattern.compile("\\s+(\\{alias\\}.)?severity\\s*(\\!\\=|\\<\\>|\\<\\=|\\>\\=|\\=|\\<|\\>)\\s*'?("
                + severities + ")'?");
    }
        ...
        if (params.containsKey("alarmId")) {
            if (params.containsKey("id")) {
                throw new IllegalArgumentException(
                                                   "Form parameters contain both the 'alarmId' and 'id' properties!  Pick a side!");
            }
    ...
    protected void translateParameters(final MultivaluedMap<String, String> params) {
        ...
        // System.err.println("tranlateSeverity: query = " + query +
        // ", pattern = " + p);
        if (query != null) {
            final Matcher m = m_severityPattern.matcher(query);
            if (m.find()) {
                // System.err.println("translateSeverity: group(1) = '" +
                // m.group(1) + "', group(2) = '" + m.group(2) +
                // "', group(3) = '" + m.group(3) + "'");
                ...
                // System.err.println("translateSeverity: " + severity + " = " +
                // onmsSeverity);

                final String newQuery = m.replaceFirst(" " + (alias == null ? "" : alias) + "severity " + comparator
                        + " " + onmsSeverity.getId());
                params.remove("query");
                params.add("query", newQuery);
                // System.err.println("translateSeverity: newQuery = '" +
                // newQuery + "'");
            } else {
