 ...
 * minor exceptions. This is so we can be sure that while we're using JRobin
 * code
 * for the parsing, we've passed values correctly. And then if we rewrite or
 * replace the parsing code with something else, we know what we expected should
 * work.
 *
 * @author cmiskell
 */
public class RrdGraphControllerTest {
    ...
    public final void setUp() throws InterruptedException {
        ...
        // Many of these tests create Calendars based on the current time, then
        // use the
        // parsing code which will assume something about when 'now' is, often
        // the current day/week
        // If we're running around midnight and the Calendar gets created on one
        // day and the parsing
        // happens on the next, you'll get spurious failures, which would be
        // really annoying and hard
        // to replicate. Initial tests show that most tests take ~100ms, so if
        // it's within 10 seconds
     ...
     * @param desc
     *            - some description of the time. Usually "start" or "end",
     *            could be others
     */
    private void assertTimestampsEqualWithEpsilon(final long expected, final long actual, final int epsilon,
            final String desc) {
    ...
    public final void testDefaultStartEndTimeSpec() {
        ...
        assertTrue("Expecting the calculated end time " + end + " to be within 1000ms of now " + now,
                   Math.abs(end - now) < 1000);
    }

    /**
     * Test the defaults if someone managed to pass through an empty string. A
     * bit paranoid, but hey, why not
     */
    @Test
    ...
    /**
     * Test the specification of just an "hour", with "yesterday" as the day,
     * for
     * both start and end, using am/pm designators
     */
    // Will fail until JRobin 1.5.13 comes out with my fix to TimeParser (not
    ...
    public final void testHourMinuteSyntax() {
        ...
        // This also exercises the test of the order of parsing (time then day).
        // If
        // that order is wrong, 8.30 could be (and was at one point) interpreted
        // as a day.month
    ...
    public final void testDateWithDots() {
        ...
        // Start is a simple one; end ensures we have our days/months around the
        // right way.
    ...
    /**
     * Test day of week specification. The expected behaviour is annoyingly
     * murky; if these tests start failing
     * give serious consideration to fixing the tests rather than the underlying
     * code.
     */
    @Test
    ...
    /**
     * Test some basic time offsets
     * NB: Due to it's use of a "day" offset, this may fail around daylight
     * savings time.
